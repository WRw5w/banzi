...
// 变量：bad=容斥累计得到的、至少被一个给定除数整除的整数个数 bad。
long long bad = 0;
// 变量：mask=用二进制位编码的当前状态 mask。
for (int mask = 1; mask < (1 << m); ++mask) {
    // 变量：l=当前子集内全部除数的最小公倍数 l。
    long long l = 1;
    // 变量：bits=当前容斥子集中被选除数的数量 bits。
    int bits = 0;
    // 变量：overflow=当前子集最小公倍数是否已超过安全上界的标记 overflow。
    bool overflow = false;
    // 变量：i=当前容斥子集 mask 中正在检查是否选中除数 a[i] 的位编号 i。
    for (int i = 0; i < m; ++i) if (mask >> i & 1) {
        ++bits;
        l = lcm_limit(l, a[i], n, overflow);
    }
    if (overflow || l > n) continue;
    // 变量：ways=1..n 中能被当前子集最小公倍数 l 整除的整数个数 ways。
    long long ways = n / l;
    if (bits & 1) bad += ways;
    else bad -= ways;
}
...
// 参数：待求模幂的底数 a；非负指数 e；幂运算使用的正模数 p。
...
// 参数：组合数 C(n,k) 的上标 n；组合数 C(n,k) 的下标 k；组合数运算使用的质数模数 p；阶乘表 fac；逆阶乘表 ifac。
...
// 参数：组合数 C(n,k) 的上标 n；组合数 C(n,k) 的下标 k；Lucas 定理使用的质数模数 p；阶乘表 fac；逆阶乘表 ifac。
...
// 接口：merge(a, m, b, n)：合并 x≡a(mod m) 与 x≡b(mod n)，不相容时返回 false；方程相容且 a,m 已更新时返回 true。
// 参数：第一个同余式余数及合并后余数的输入输出引用 a；第一个同余式模数及合并后模数的输入输出引用 m；第二个同余式的余数 b；第二个同余式的正模数 n。
bool merge(long long& a, long long& m,
           long long b, long long n) {
    // 旧方程：x ≡ a (mod m)；新方程：x ≡ b (mod n)。
    // 合并后仍用 a,m 表示一个最小非负解和新的最小公倍数模数。
    // 变量：x=exgcd 写回的 Bézout 系数 x；y=exgcd 写回的 Bézout 系数 y。
    long long x, y;
    // 变量：g=递归返回的 gcd(a,b) 值 g。
...
int gauss_mod(vector<vector<long long>> a, long long mod) {
    ...
    // 变量：n=容器 a 的元素数量 n；m=增广矩阵 a 的列数 m；row=当前高斯消元主元行 row。
    ...
    for (int col = 0; col < m && row < n; ++col) {
        ...
        // 变量：i=高斯消元中当前用主元行消去的其它方程行 i。
        for (int i = 0; i < n; ++i) if (i != row) {
            // 变量：k=高斯消元用当前行消去主元列的倍数 k。
            long long k = a[i][col];
            // 用当前主元行消掉其它行的这一列。
            // 变量：j=高斯消元中从主元列 col 开始更新的增广矩阵列下标 j。
            for (int j = col; j < m; ++j)
...
// 接口：linear_sieve_mu(n)：线性筛出 1..n 的莫比乌斯函数 mu；不返回值。
// 参数：筛法计算的正整数上界 n。
...
// 参数：离散对数方程 a^x=b (mod mod) 的底数 a；目标剩余 b；正模数 mod。
long long bsgs(long long a, long long b, long long mod) {
    ...
    cur = b;
    // 变量：i=BSGS 巨步枚举次数 i，对应指数块 i*m。
    for (long long i = 0; i <= m; ++i) {
        // 变量：it=容器 baby 中当前查找结果的迭代器 it。
...
long long tonelli_shanks(long long n, long long p) {
    ...
    // 变量：c=Tonelli-Shanks 当前 2 幂阶生成元 c。
    long long c = mod_pow(z, q, p);
    // 变量：x=Tonelli-Shanks 当前平方根候选 x。
    long long x = mod_pow(n, (q + 1) / 2, p);
    // 变量：t=Tonelli-Shanks 当前误差项 t。
    long long t = mod_pow(n, q, p);
    // 变量：m=Tonelli-Shanks 当前误差项的 2 幂阶上界 m。
    int m = s;
    while (t != 1) {
...
// 参数：待做实数高斯消元的增广矩阵 a；接收唯一解的输出数组 ans。
int gauss_real(vector<vector<long double>> a,
               vector<long double>& ans) {
    // a[i][0..m-1] 是系数，a[i][m] 是常数项；这里默认方程数=未知数数。
    // 返回 0=无解，1=唯一解，2=多解。
    // 变量：n=容器 a 的元素数量 n；m=方阵 a 的列数 m；row=当前高斯消元主元行 row。
    int n = a.size(), m = n, row = 0;
    // 变量：col=当前高斯消元处理的列 col。
    for (int col = 0; col < m && row < n; ++col) {
        // 变量：p=高斯消元为当前列搜索到的主元行 p。
        int p = row;
        // 变量：i=实数高斯消元中在主元行下方寻找最大绝对值元的候选行 i。
        ...
        for (int i = 0; i < n; ++i) if (i != row) {
            ...
            // 变量：j=实数高斯消元中从主元列到常数列的更新下标 j。
            for (int j = col; j <= m; ++j)
                a[i][j] -= k * a[row][j];
        }
        ++row;
    }
    // 变量：i=实数高斯消元结束后检查一致性的剩余方程行 i。
    for (int i = row; i < n; ++i) {
        // 变量：all_zero=方程当前行系数是否全为 0 的标记 all_zero。
        bool all_zero = true;
        // 变量：j=实数高斯消元中检查剩余方程行是否全零的系数列下标 j。
        for (int j = 0; j < m; ++j)
