...
vector<long long> ans;       // ans[u]=以 u 为根的子树询问答案；可容纳频次平方和。
vector<int> cnt;             // cnt[c]=当前保留节点中颜色 c 的出现次数。
long long cur_answer = 0; // 示例：维护所有颜色频次的平方和。

// 接口：dfs_size(u, fa)：计算以 u 为根的子树大小 sz，并选出子树大小最大的重儿子 son；不返回值。
// 参数：当前顶点编号 u；父节点 fa。
...
// 接口：add_node(u, delta)：把顶点 u 的颜色计数增加 delta；不返回值。
// 参数：顶点编号 u；颜色出现次数的增量 delta，加入为 1，移除为 -1。
...
// 接口：add_subtree(u, fa, delta)：遍历 u 的整棵子树且不回到父节点 fa，按 delta 把各顶点贡献加入或移出统计；会修改当前统计。
// 参数：当前顶点编号 u；父节点 fa；本次增量 delta。
...
// 接口：dsu(u, fa, keep)：按 keep 参数执行 DSU on Tree；keep=false 时清除 u 子树贡献；不返回值。
// 参数：当前顶点编号 u；父节点 fa；是否保留 u 子树统计贡献 keep。
void dsu(int u, int fa, bool keep) {
    ...
    // 重子树已经保留、轻子树刚刚加入，此处只加入 u 自己。
    add_node(u, +1);

    // 当前集合恰好是 u 的整棵子树，读取题目答案。
    ans[u] = cur_answer; // 换成题目要求的统计量
...
\begin{lstlisting}[language={C++}]
// 接口：dfs_len(u, fa)：计算 u 子树最大深度并选出最长链儿子；不返回值。
// 参数：当前顶点编号 u；父节点 fa。
...
// 接口：dfs_chain(u, top)：沿最长链优先递归并填写每个点的链顶 top；不返回值。
// 参数：当前顶点编号 u；顶点 u 所属长链的链顶编号 top。
...
\begin{lstlisting}[language={C++}]
// 接口：collect(u, fa, d, ds)：收集未删除子树中各节点到当前点分治重心的距离；不返回值。
// 参数：当前顶点编号 u；父节点 fa；距离 d；接收当前子树距离的输出数组 ds。
...
// 接口：decompose(entry)：找出 entry 所在未删除连通块的重心并递归分解各子块；不返回值。
// 参数：当前连通块入口 entry。
...
\begin{quote}
说明：当前代码只展示了点分治的分解和距离收集框架，没有给出完整的重心查找、
查询容器、修改和答案接口。因此这里不伪造``输入/标准输出''样例；等这些接口补全后，
测试必须至少包含单点、链、星形树、半径为零和跨重心重复计数五类情况。
\end{quote}
