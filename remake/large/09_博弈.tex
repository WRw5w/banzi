...
// 接口：nim_first_win(piles)：返回标准 Nim 当前局面是否先手必胜；全部堆石子数异或和非零时返回 true。
// 参数：各堆石子数 piles。
bool nim_first_win(const vector<int>& piles) {
    int x = 0; // 整个无偏 Nim 游戏的 SG 异或和。
    // 变量：v=从 piles 枚举的单堆石子数 v。
    for (int v : piles) x ^= v;
    return x != 0;
}

// 接口：bash_first_win(n, k)：返回 Bash 取石子局面是否先手必胜；n 不是 k+1 的倍数时返回 true。
...
\begin{lstlisting}[language={C++}]
// 接口：misere_nim_first_win(piles)：返回反常 Nim 当前局面是否先手必胜；按全 1 堆特判或普通 Nim 异或条件判胜。
// 参数：各堆石子数 piles。
bool misere_nim_first_win(const vector<int>& piles) {
    // 只有“所有非空堆都是 1”时，最后一步输的规则会改变公式。
    // 变量：all_one=所有堆是否都只有 1 个石子的标记 all_one。
    bool all_one = true;
    // 变量：x=反常 Nim 全部堆石子数的异或和 x；ones=石子数恰为 1 的堆数量 ones。
    int x = 0, ones = 0;
    // 变量：v=从 piles 枚举的单堆石子数 v。
    for (int v : piles) {
        if (v > 0) all_one &= (v == 1); // 空堆不参与“所有非空堆均为 1”的判断。
        x ^= v;
        ones += (v == 1);
    }
...
// 接口：first_win(starts)：返回若干独立子游戏 SG 值异或和是否非零；异或和非零时返回 true。
// 参数：若干独立子游戏的起始状态 starts。
bool first_win(const vector<int>& starts) {
    // 只有子游戏互不影响时才能异或；共享资源的游戏不能套这里。
    // 变量：total=starts 中全部独立子游戏 SG 值的累计异或和 total。
    int total = 0;
    // 变量：s=从容器 starts 枚举的元素 s。
...
\begin{lstlisting}[language={C++}]
// 接口：solve_subtraction(n, moves)：返回单堆剩余 n 个石子时、每步取数来自 moves 的减法游戏 SG 值；返回类型 int。
// 参数：待求 SG 值的石子数 n；每步允许取走的石子数集合 moves。
