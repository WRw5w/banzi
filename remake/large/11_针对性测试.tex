...
\subsubsection{任意精度整数：符号、整除、超 128 位与模幂}

\begin{lstlisting}
1) 10^50 + 12345                         -> 十进制输入输出不丢位
2) a 和 -a 分别除以 97                   -> 都满足 a=(a/b)*b+a%b
3) 10^50                                 -> 明显超过 __int128
4) (-2)^5 mod 13                         -> 7
\end{lstlisting}

第三组抓误用 \passthrough{\lstinline!__int128!}；第四组抓负底数未先归一化。
若题目要求余数位于 \passthrough{\lstinline![0,b)!}，不能直接把负数的
\passthrough{\lstinline!a\%b!} 当答案。
