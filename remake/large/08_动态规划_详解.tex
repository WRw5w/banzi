...
// 接口：dfs(pos, tight, started, st)：返回给定数位 DP 状态的合法后缀方案数；返回类型 long long。
// 参数：当前处理到的十进制数位下标 pos；前缀是否仍贴住上界 tight；是否已经放置非前导零数字 started；处理当前前缀后所在的数字自动机状态 st，范围 [0,STATE)。
long long dfs(int pos, bool tight, bool started, int st) {
    ...
    // 变量：res=当前数位 DP 状态累计的合法后缀方案数 res。
    long long res = 0;
    // 变量：up=当前数位 pos 可枚举数字的上界 up；贴住原数时为 digit[pos]，否则为 9。
    ...
    for (int d = 0; d <= up; ++d) {
        ...
        // 变量：nst=在自动机状态 st 后放置数字 d 得到的下一自动机状态 nst；仍处于前导零时保持起点 0。
...
\begin{lstlisting}[language={C++}]
// 数值域：0<=K<31，且每个子集总和都必须可由 f 的元素类型表示；否则把 f 改为 __int128。
...
\begin{lstlisting}[language={C++}]
// 变量：q=按未来转移优劣维护候选 DP 下标的单调双端队列 q。
deque<int> q;
...
struct Line {
    ...
    // 接口：value(x)：在 __int128 中精确返回直线 k*x+b 在横坐标 x 处的值。
    // 参数：任意 long long 查询横坐标 x。
    __int128 value(long long x) const { return (__int128)k * x + b; }
};
...
deque<Line> hull;
// 接口：useless(a, b, c)：判断中间候选直线 b 是否可从凸包队列删除；b 永远不会最优时返回 true。
...
// 接口：add_line(line)：按单调斜率加入候选直线，并从队尾删除永不最优的直线；会修改当前结构；多测时重新构造或显式清空成员。
// 参数：待插入的候选直线 line。
...
// 接口：divide_conquer_dp(L, R, optL, optR)：计算 DP 下标闭区间 [L,R]，决策只搜索 [optL,optR]，并把结果写入全局 dp；无返回值。
...
void divide_conquer_dp(int L, int R, int optL, int optR) {
    ...
    // 变量：j=分治 DP 在允许区间 [optL,optR] 内枚举的候选决策点 j。
...
for (int mask = 1; mask < (1 << K); ++mask) {
    ...
    for (int sub = (mask - 1) & mask; sub; sub = (sub - 1) & mask) {
        // 变量：other=mask 中除 sub 之外的互补子集 other。
        int other = mask ^ sub;
        if (sub > other) continue; // 对称拆分只做一次，常数更小。
        // 变量：v=Steiner 树状态压缩 DP 当前汇合的图顶点 v。
        for (int v = 0; v < n; ++v)
