...
vector<long long> gen(const vector<long long>& a, int l, int r,
                      long long limit) {
    // 变量：res=当前半区枚举得到的全部子集和 res。
    vector<long long> res{0};
    // 变量：i=当前半开区间 [l,r) 内顺序枚举的元素下标 i。
    for (int i = l; i < r; ++i) {
        // 变量：old=加入当前元素 a[i] 生成新子集和之前 res 中已有的元素数量 old。
        int old = (int)res.size();
        // 变量：j=组合生成中复制上一轮已有结果的下标 j。
        for (int j = 0; j < old; ++j)
...
// 变量：pre=原数组 a 的 1-based 前缀和数组 pre；diff=记录区间增量端点标记的一维差分数组 diff。
vector<long long> pre(n + 1), diff(n + 2);
// 变量：i=正在计算前缀和 pre[i] 的 1-based 数组位置 i。
for (int i = 1; i <= n; ++i) pre[i] = pre[i - 1] + a[i];
// 接口：range_sum(l, r)：返回 1-based 闭区间 [l,r] 的前缀和差。
// 参数：查询闭区间左端点 l；查询闭区间右端点 r。
// 变量：range_sum=返回指定区间和的局部 lambda range_sum。
auto range_sum = [&](int l, int r) { return pre[r] - pre[l - 1]; };

// 接口：range_add(l, r, v)：在差分数组中记录 1-based 闭区间 [l,r] 增加 v。
// 参数：修改闭区间左端点 l；修改闭区间右端点 r；区间增加量 v。
auto range_add = [&](int l, int r, long long v) {
    diff[l] += v;
    diff[r + 1] -= v;
};
// 变量：l=差分更新区间左端点 l；r=差分更新区间右端点 r；v=区间增加量 v。
for (auto [l, r, v] : updates) range_add(l, r, v);
// 变量：cur=扫描到当前位置时由差分数组累加得到的区间增量 cur。
long long cur = 0;
// 变量：i=差分数组还原时当前累加并写回的 1-based 位置 i。
for (int i = 1; i <= n; ++i) {
    cur += diff[i];
    a[i] += cur;
}
...
// 变量：distinct=当前窗口内不同值的数量 distinct；left=滑动窗口左端点 left；answer=当前维护的答案 answer。
...
while (r - l > 3) {
    // 变量：m1=整数三分区间内靠左的三等分点 m1。
    long long m1 = l + (r - l) / 3;
    // 变量：m2=整数三分区间内靠右的三等分点 m2。
    long long m2 = r - (r - l) / 3;
    if (f(m1) < f(m2)) l = m1 + 1; // 求最大值
    else r = m2 - 1;
}
// 变量：ans=整数三分结束后在剩余区间暴力检查得到的最大函数值 ans。
long long ans = LLONG_MIN;
// 变量：x=暴力检查凹凸函数值的当前整数横坐标 x。
...
sort(intervals.begin(), intervals.end(),
     // 接口：匿名 lambda(x, y)：按区间右端点从小到大排序，供不相交区间贪心选择。
     // 参数：第一个候选区间 x；第二个候选区间 y。
     [](auto &x, auto &y) { return x.second < y.second; });
// 变量：last=上一个被选区间的右端点 last，初值表示尚未选择；chosen=当前已选择的两两不相交区间数量 chosen。
int last = numeric_limits<int>::min(), chosen = 0;
// 变量：l=当前候选区间的左端点 l；r=当前候选区间的右端点 r。
for (auto [l, r] : intervals) {
...
sort(jobs.begin(), jobs.end(),
     // 接口：匿名 lambda(x, y)：按任务截止时间从小到大排序。
     // 参数：第一个候选任务 x；第二个候选任务 y。
     [](auto &x, auto &y) { return x.deadline < y.deadline; });
// 变量：chosen=维护当前已选任务收益的最小堆 chosen。
...
// 变量：q=差分约束 SPFA 中保存待松弛顶点的队列 q。
queue<int> q;
// 变量：s=差分约束初始入队的顶点编号 s。
for (int s = 0; s < n; ++s) q.push(s), inq[s] = 1;
// 变量：ok=当前条件是否仍成立 ok。
bool ok = true;
while (!q.empty() && ok) {
    // 变量：u=差分约束 SPFA 当前出队并准备松弛出边的顶点编号 u。
...
\begin{lstlisting}[language={C++}]
// 变量：nextGreater=每个位置右侧第一个更大元素下标 nextGreater；st=维护下标且对应值单调递减的栈 st。
vector<int> nextGreater(n, -1), st;
// 变量：i=单调栈正在处理并确定右侧更大元素的数组位置 i。
for (int i = 0; i < n; ++i) {
...
\begin{lstlisting}[language={C++}]
// 变量：q=按数组值单调递减维护候选下标的双端队列 q。
deque<int> q;
// 变量：i=滑动窗口右端新加入单调队列的数组位置 i。
for (int i = 0; i < n; ++i) {
...
// 接口：id(x)：返回原坐标 x 在有序离散数组 xs 中的 0-based 下标。
// 参数：待离散化的原坐标 x。
auto id = [&](long long x) {
...
// 变量：safe_product=用 __int128 计算并取模后安全转回 long long 的乘积 safe_product。
