...
\begin{lstlisting}[language={C++}]
// 接口：dfs(pos, sum)：从当前节点/位置继续深度优先处理，并写入本节状态数组；无返回值；结果写入引用参数或当前结构状态。
...
\begin{lstlisting}[language={C++}]
// 接口：gen(a, l, r)：枚举 a 的半开下标区间 [l,r) 的所有子集和；返回类型 vector<long long>。
// 参数：输入数组 a；半开区间左端点 l；半开区间右端点 r。
...
\begin{lstlisting}[language={C++}]
// 接口：minimize_max_segment_sum(a, k)：把非负数组分成至多 k 个非空连续段，返回最小的最大段和。
...
// 任一操作数为 0 时 lcm 为 0，不能先除以 gcd(0,0)。
// 变量：l=a 与 b 的最小公倍数。
