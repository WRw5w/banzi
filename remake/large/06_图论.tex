...
\begin{lstlisting}[language={C++}]
// 变量：q=Kahn 拓扑排序中保存当前入度为 0 顶点的队列 q。
queue<int> q;
// 变量：u=初始化 Kahn 队列时当前检查入度的顶点编号 u。
for (int u = 0; u < n; ++u) if (!indeg[u]) q.push(u);
// 变量：topo=拓扑序列 topo。
vector<int> topo;
while (!q.empty()) {
    // 变量：u=Kahn 拓扑排序当前出队并删除出边的顶点编号 u。
...
\begin{lstlisting}[language={C++}]
// 变量：q=0-1 BFS 中按新距离优先级保存待扩展顶点的双端队列 q。
deque<int> q;
// 变量：d=从指定源点到各顶点的距离数组 d。
vector<int> d(n, INF); d[s] = 0; q.push_front(s);
while (!q.empty()) {
    // 变量：u=0-1 BFS 当前从队首取出并松弛出边的顶点编号 u。
    int u = q.front(); q.pop_front();
    // 变量：v=从 zero_one_graph[u] 枚举的相邻顶点 v；w=连接该顶点的边权 w。
...
struct MstEdge {
    ...
    long long w;    // 边权；允许为负。
    // 接口：operator<(o)：按边权升序比较两条边，供 Kruskal 排序；当当前边权 w 小于 o.w 时返回 true。
    // 参数：与当前边比较权值的另一条最小生成树候选边 o。
...
struct KruskalDSU {
  ...
  // 参数：Kruskal 并查集管理的顶点数 n。
    ...
    // 接口：unite(x, y)：合并两个元素所在集合；原本同集合时返回 false；实际合并两个集合时返回 true。
    // 参数：第一个待合并元素编号 x；第二个待合并元素编号 y。
...
// 参数：无向图顶点数 n；无向带权边集合 edges。
MstResult kruskal(int n, vector<MstEdge> edges) {
    sort(edges.begin(), edges.end());
    // 变量：dsu=Kruskal 扫描边时维护连通块的并查集对象 dsu。
    KruskalDSU dsu(n);
    // 变量：ans=Kruskal 当前生成森林的总权、已选边数、连通状态和边集 ans。
    MstResult ans{0, 0, n <= 1, {}};
    // 变量：e=从容器 edges 枚举的元素 e。
...
// 参数：Prim 使用的 0-based 加权无向图邻接表 g；每项为 (终点,边权)。
pair<bool, long long> prim(
    ...
    // 变量：cost=当前累计费用 cost。
    long long cost = 0;
    // 变量：cnt=Prim 已经加入生成树的顶点数量 cnt。
    int cnt = 0;
    while (!pq.empty()) {
        auto [w, u] = pq.top(); pq.pop();
        if (used[u]) continue;
        used[u] = true; cost += w; ++cnt;
        // 变量：v=从 g[u] 枚举的相邻顶点 v；c=连接该顶点的边权 w。
        for (auto [v, c] : g[u])
            if (!used[v] && c < best[v])
...
struct KruskalTree {
    ...
    // 接口：build(n_, edges)：按边权建立 Kruskal 重构树并预处理深度、子树大小和倍增祖先；完成初始化；多测时每组重新调用。
    // 参数：原图点数 n_；输入边集合 edges。
    void build(int n_, vector<Edge> edges) { // 顶点编号 1..n
        n = n_; tot = n;
        // 变量：cap=Kruskal 重构树为原点和合并点预留的数组容量 cap。
        int cap = 2 * n + 5;
        LOG = 1;
        ...
        child.assign(cap, {});
        // 变量：i=Kruskal 重构树初始化子树大小的原图顶点编号 i。
        for (int i = 1; i <= n; ++i) sz[i] = 1;
        sort(edges.begin(), edges.end(),
             // 接口：匿名 lambda(a, b)：按边权从小到大排列 Kruskal 候选边。
             // 参数：第一条候选边 a；第二条候选边 b。
             [](const Edge &a, const Edge &b) { return a.w < b.w; });
        // 变量：e=从容器 edges 枚举的元素 e。
        for (auto e : edges) {
            // 变量：x=Kruskal 当前边一端所在连通块的代表元 x；y=Kruskal 当前边一端所在连通块的代表元 y。
        ...
        // 变量：st=遍历 Kruskal 重构森林并计算深度的顶点栈 st。
        vector<int> st;
        // 变量：u=当前检查是否为 Kruskal 重构森林根的节点编号 u。
        for (int u = 1; u <= tot; ++u) if (dsu[u] == u) {
            dep[u] = 1; st.push_back(u); // 不连通图得到森林
        }
        while (!st.empty()) {
            // 变量：u=当前从 Kruskal 重构森林遍历栈弹出的节点编号 u。
            int u = st.back(); st.pop_back();
            // 变量：v=从 child[u] 枚举的相邻顶点 v。
            for (int v : child[u]) dep[v] = dep[u] + 1, st.push_back(v);
        }
        // 变量：j=Kruskal 重构树正在预处理的 2^j 级祖先层编号 j。
        for (int j = 1; j < LOG; ++j)
            // 变量：u=当前填写倍增祖先表的 Kruskal 重构树节点编号 u。
            for (int u = 1; u <= tot; ++u)
                up[j][u] = up[j - 1][up[j - 1][u]];
    }
    // 接口：lca(u, v)：返回树上 u,v 的最近公共祖先；返回类型 int。
    // 参数：当前顶点编号 u；与 u 配对的顶点编号 v。
    int lca(int u, int v) const {
        if (dep[u] < dep[v]) swap(u, v);
        // 变量：d=把较深顶点提升到同深度所需的层数差 d。
        int d = dep[u] - dep[v];
        // 变量：j=按深度差 d 的置位情况提升顶点 u 时检查的倍增层编号 j。
        for (int j = 0; j < LOG; ++j) if (d >> j & 1) u = up[j][u];
        if (u == v) return u;
        // 变量：j=Kruskal 重构树 LCA 中从高到低比较祖先的倍增层编号 j。
    ...
    // 参数：当前顶点编号 u；允许的阈值 lim。
    int component_node(int u, long long lim) const {
        // 变量：j=按阈值 lim 向上跳转时从高到低检查的倍增层编号 j。
        for (int j = LOG - 1; j >= 0; --j) {
            // 变量：a=顶点 u 的 2^j 级祖先 a。
            int a = up[j][u];
            if (a && val[a] <= lim) u = a;
        }
        return u;
...
\begin{lstlisting}[language={C++}]
// 变量：timer=强连通分量 Tarjan DFS 使用的递增时间戳 timer；dfn=DFS 首次访问时间戳 dfn；low=Tarjan 最低可达时间戳 low；stk=尚未归入强连通分量的顶点栈 stk；in=顶点当前是否在 Tarjan 栈内的标记 in；comp=每个顶点所属强连通分量编号 comp；cc=当前已经找到的强连通分量数量 cc。
int timer = 0; vector<int> dfn(n), low(n), stk, in(n), comp(n); int cc = 0;
// 接口：tarjan(u)：从 u 开始求强连通分量的 Tarjan DFS；不返回值。
// 参数：当前顶点编号 u。
void tarjan(int u) {
...
\begin{lstlisting}[language={C++}]
// 变量：timer=无向图桥 Tarjan DFS 使用的递增时间戳 timer；dfn=DFS 首次访问时间戳 dfn；low=Tarjan 最低可达时间戳 low；bridges=已经找到的桥边端点集合 bridges。
int timer = 0; vector<int> dfn(n), low(n); vector<pair<int, int>> bridges;
// 接口：dfs(u, parent_edge)：从 u 继续无向图 Tarjan，填写 dfn/low 并收集桥；不返回值。
// 参数：当前顶点编号 u；DFS 进入 u 所用的无向边编号 parent_edge。
void dfs(int u, int parent_edge) {
    dfn[u] = low[u] = ++timer;
    // 变量：v=当前边的终点 v；id=当前无向边的唯一编号 id，用于跳过 DFS 父边。
    for (auto [v, id] : g[u]) {
...
\begin{lstlisting}[language={C++}]
// 变量：color=二分图 BFS 中每个顶点的 0/1 颜色数组 color；-1 表示尚未染色。
vector<int> color(n, -1);
// 变量：ok=当前条件是否仍成立 ok。
bool ok = true;
// 变量：s=当前新连通分量开始二分图染色的起点 s。
for (int s = 0; s < n; ++s) if (color[s] < 0) {
    // 变量：q=当前连通块二分图染色所用的 BFS 顶点队列 q。
    queue<int> q; q.push(s); color[s] = 0;
    while (!q.empty()) {
        // 变量：u=二分图染色 BFS 当前出队并检查邻点的顶点编号 u。
        int u = q.front(); q.pop();
        // 变量：v=当前边的终点 v。
        for (int v : g[u]) {
...
// 变量：level=Dinic 层次图中的顶点层数 level；it=Dinic 中每个顶点下一条待尝试边的当前弧下标 it。
vector<int> level, it;
// 接口：add_edge(u, v, c)：向 Dinic 残量网络加入容量 c 的边及零容量反向边；会修改当前结构；多测时重新构造或显式清空成员。
// 参数：当前顶点编号 u；与 u 配对的顶点编号 v；Dinic 正向边容量 c。
void add_edge(int u, int v, long long c) {
    // 变量：a=正向残量边 a；b=配套的反向残量边 b。
    Edge a{v, (int)fg[v].size(), c}, b{u, (int)fg[u].size(), 0};
    fg[u].push_back(a); fg[v].push_back(b);
}
// 接口：bfs(s, t)：从源点 s 建立 Dinic 层次图；当残量网络中汇点 t 可达时返回 true。
// 参数：层次图的源点编号 s；层次图的汇点编号 t。
bool bfs(int s, int t) {
    // 变量：q=Dinic 建立层次图时保存待扩展顶点的 BFS 队列 q。
    fill(level.begin(), level.end(), -1); queue<int> q;
    level[s] = 0; q.push(s);
    // 变量：u=Dinic 层次图 BFS 当前出队并扫描残量边的顶点编号 u。
...
// 接口：dfs(u, t, f)：在 Dinic 层次图上从 u 向汇点 t 推送至多 f 的阻塞流；返回类型 long long。
// 参数：当前顶点编号 u；增广目标汇点编号 t；本次 DFS 允许继续发送的流量 f。
...
// 接口：add_edge(u, v, cap, cost)：向最小费用流残量网络加入容量 cap、费用 cost 的边及其反向边；会修改当前结构；多测时重新构造或显式清空成员。
// 参数：当前顶点编号 u；与 u 配对的顶点编号 v；边容量 cap；每单位流费用 cost。
void add_edge(int u, int v, int cap, int cost) {
    // 变量：a=正向残量边 a。
    MCEdge a{v, (int)mg[v].size(), cap, cost};
    // 变量：b=与正向费用流边配对的反向残量边 b。
...
// 参数：费用流源点编号 s；费用流汇点编号 t；希望发送的流量 need。
pair<int, long long> min_cost_flow(int s, int t, int need) {
    // 变量：flow=当前已经发送的流量 flow；cost=当前累计费用 cost。
    int flow = 0; long long cost = 0;
    // 变量：pot=费用流顶点势能数组 pot；dist=当前维护的距离 dist。
...
\begin{lstlisting}[language={C++}]
// 变量：timer=DFS 使用的递增时间戳 timer；tin=DFS 进入每个节点的时间戳 tin；tout=DFS 离开每个节点的时间戳 tout；depth=DFS 树中每个顶点的深度数组 depth；euler=欧拉路径最终顶点序列 euler。
int timer = 0; vector<int> tin(n), tout(n), depth(n), euler(n + 1);
// 接口：dfs(u, p)：为以 u 为根的子树填写 DFS 序、深度和子树区间；不返回值。
...
// 接口：init_lca(u, p)：从 u 开始 DFS，原地填写各顶点深度 dep 与倍增祖先表 up；无返回值。
// 参数：当前节点 u；u 的父节点 p，根节点的父节点传 0。
void init_lca(int u, int p) {
    up[u][0] = p;
    // 变量：j=为顶点 u 递推 up[u][j] 时当前填写的倍增层编号 j。
...
int lca(int u, int v) {
    ...
    if (u == v) return u;
    // 变量：j=LCA 查询中从高到低比较 u、v 祖先的倍增层编号 j。
    for (int j = LOG - 1; j >= 0; --j)
...
void add_path(int u, int v) {
    // 变量：w=树上路径 u-v 的最近公共祖先节点 w。
    int w = lca(u, v);
    ++diff[u]; ++diff[v]; --diff[w];
    if (up[w][0]) --diff[up[w][0]];
}
// 接口：collect(u, p)：后序累加树上路径差分，把孩子贡献汇总到父节点；不返回值。
// 参数：当前顶点编号 u；当前节点 u 的父节点 p。
...
pair<int, long long> farthest(int s) {
    // 变量：d=从起点 s 到各树顶点的距离数组 d；q=树上最远点搜索中保存待扩展顶点的 BFS 队列 q。
    vector<long long> d(n, -1); queue<int> q;
    d[s] = 0; q.push(s);
    // 变量：best=目前找到的最优值 best。
    int best = s;
    // 变量：u=树上最远点 BFS 当前出队并扩展邻边的顶点编号 u。
    while (!q.empty()) { int u = q.front(); q.pop();
        if (d[u] > d[best]) best = u;
        // 变量：v=从 weighted_tree[u] 枚举的相邻顶点 v；w=连接该顶点的边权 w。
...
\begin{lstlisting}[language={C++}]
// 接口：tree_dp(u, p)：自底向上计算以 u 为根的树形 DP 状态；不返回值。
// 参数：当前顶点编号 u；当前节点 u 的父节点 p。
...
\begin{lstlisting}[language={C++}]
// 接口：dfs1(u, p)：计算 HLD 所需的父亲、深度、子树大小和重儿子；不返回值。
// 参数：当前顶点编号 u；当前节点 u 的父节点 p。
...
// 接口：dfs2(u, topf)：沿重链优先编号并填写链顶 top；不返回值。
// 参数：当前顶点编号 u；当前重链链顶 topf。
...
long long path_sum(int u, int v) {
    // 变量：ans=重链剖分查询路径 u-v 时累计的各链段区间和 ans。
    long long ans = 0;
    while (top[u] != top[v]) {
...
// 参数：当前顶点编号 u；当前节点 u 的父节点 p；当前连通块总大小 total。
...
// 接口：divide(entry)：找出 entry 所在未删除连通块的重心、标记 removed 并递归分解各剩余子块；无返回值。
// 参数：当前连通块入口 entry。
void divide(int entry) {
    // 变量：c=当前未删除连通块的重心顶点 c。
    int c = get_centroid(entry, 0, get_size(entry, 0));
    removed[c] = true;
    // 统计经过 c 的贡献
    // 变量：v=从 tree[c] 枚举的相邻顶点 v。
...
// 变量：answer=当前维护的答案 answer。
vector<int> answer(n + 1);
int distinct = 0; // 示例维护量：当前集合中出现过的颜色数。

// 接口：dfs_size(u, p)：计算以 u 为根的子树大小 sz，并选出子树大小最大的重儿子 heavy；不返回值。
// 参数：当前顶点编号 u；当前节点 u 的父节点 p。
...
// 接口：add_subtree(u, p, delta)：遍历 u 的整棵子树且不回到父节点 p，按 delta=1 加入颜色频次或按 delta=-1 移除颜色频次；会修改当前统计。
// 参数：当前顶点编号 u；当前节点 u 的父节点 p；本次增量 delta。
...
// 接口：dsu_on_tree(u, p, keep)：按 keep 参数执行 DSU on Tree；keep=false 时清除贡献；不返回值。
// 参数：当前顶点编号 u；当前节点 u 的父节点 p；是否保留 u 子树统计贡献 keep。
...
\begin{lstlisting}[language={C++}]
// 变量：parent2=长链剖分中每个顶点的父节点数组 parent2；depth2=长链剖分中每个顶点的深度数组 depth2；heavy_len=从每个点向下的最长链长度 heavy_len。
...
// 接口：long_chain_dfs1(u, p)：计算子树最大深度并选择最长链儿子；不返回值。
// 参数：当前顶点编号 u；当前节点 u 的父节点 p。
...
// 接口：long_chain_dfs2(u, topf)：沿最长链儿子优先递归并填写每个顶点所属链顶 top2；不返回值。
// 参数：当前顶点编号 u；当前重链链顶 topf。
...
struct LCT {
  ...
  // 参数：Link-Cut Tree 管理的 1-based 顶点数量 n。
    LCT(int n): t(n + 1) {}
    // 接口：is_root(x)：判断 x 是否是当前 splay 辅助树的根；x 没有辅助树父节点或不作为父节点的直接儿子时返回 true。
    // 参数：Link-Cut Tree 中的原树顶点编号 x。
    bool is_root(int x) const {
        // x 没有 splay 父亲，或不是父亲的直接儿子时，是辅助树根。
        // 变量：f=t[x].fa 保存的节点 x 当前辅助树父节点编号 f。
        int f = t[x].fa;
        return !f || (t[f].ch[0] != x && t[f].ch[1] != x);
    }
    // 接口：pull_sum(x)：由两个 splay 儿子和自身点权重算 t[x].sum；不返回值。
    // 参数：Link-Cut Tree 中的原树顶点编号 x。
    void pull_sum(int x) {
        // 修改儿子后必须重新计算聚合信息。
        t[x].sum = t[x].val + t[t[x].ch[0]].sum + t[t[x].ch[1]].sum;
    }
    // 接口：apply_rev(x)：交换 x 的左右儿子并翻转路径反转标记；不返回值。
    // 参数：Link-Cut Tree 中的原树顶点编号 x。
    ...
    // 接口：push(x)：把 LCT 辅助树节点 x 的路径反转标记下传给两个儿子；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：Link-Cut Tree 中的原树顶点编号 x。
    void push(int x) {
    ...
    void rotate(int x) {
        ...
        // 变量：y=LCT 旋转中节点 x 的父节点 y；z=LCT 旋转中节点 y 的父节点 z；dx=节点 x 是 y 的右儿子时为 1 的方向标记 dx；b=旋转后要接到 y 下方的子树根 b。
    ...
    void splay(int x) {
        ...
        // 变量：st=splay 前按自底向上顺序收集的辅助树祖先栈 st。
        vector<int> st{ x };
        // 变量：y=从 x 沿辅助树父链上溯并收集祖先的当前节点 y。
        for (int y = x; !is_root(y); y = t[y].fa) st.push_back(t[y].fa);
        while (!st.empty()) push(st.back()), st.pop_back();
        while (!is_root(x)) {
            // 变量：y=LCT 旋转中节点 x 的父节点 y；z=LCT 旋转中节点 y 的父节点 z。
    ...
    // 接口：split(x, y)：通过 makeroot(x)、access(y)、splay(y) 暴露原树路径 x..y，使该路径由 y 的辅助树维护；无返回值。
    // 参数：待暴露路径的第一个原树顶点 x；待暴露路径的第二个原树顶点 y。
    void split(int x, int y) {
        // 之后 y 的 splay 子树正好维护原树路径 x..y。
        makeroot(x); access(y); splay(y);
    }
    // 接口：link(x, y)：连接原本不连通的 x,y；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：Link-Cut Tree 中的原树顶点编号 x；Link-Cut Tree 中的原树顶点编号 y。
