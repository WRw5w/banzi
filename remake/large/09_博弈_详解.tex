...
// 接口：grundy(u)：返回状态参数对应的 Sprague-Grundy 值并记忆化；返回类型 int。
// 参数：顶点编号 u。
int grundy(int u) {
...
nim_move(const vector<unsigned long long>& a) {
    ...
    // 变量：all=标准 Nim 当前局面所有堆石子数的异或和 all。
    ...
    // 变量：i=Nim 当前尝试执行必胜移动的石子堆下标 i。
    for (int i = 0; i < (int)a.size(); ++i) {
        // 只改变第 i 堆后，希望新异或和变成 0，目标大小必须是 a[i]^all。
        // 变量：target=使操作后 Nim 异或和为 0 时第 i 堆应剩余的石子数 target。
...
\begin{lstlisting}[language={C++}]
// 接口：bash_win(n, k)：判断每次可取 1..k 个时 n 个石子的局面是否先手胜；n 不是 k+1 的倍数时返回 true。
// 参数：Bash 游戏当前石子数 n；每步最多取走的石子数 k。
...
\begin{lstlisting}[language={C++}]
// 接口：misere_nim_win(a)：判断反常 Nim（取走最后石子者输）是否先手胜；全为 1 时堆数为偶数，否则总异或非 0 时返回 true。
// 参数：反常 Nim 各堆石子数 a。
...
\begin{lstlisting}[language={C++}]
// 接口：staircase_nim_win(a)：判断阶梯 Nim 局面是否先手胜；奇数层石子数的异或和非 0 时返回 true。
// 参数：阶梯 Nim 每层石子数 a。
...
// 参数：要预处理的最大单堆石子数 N；每步允许取走的石子数集合 moves。
vector<int> subtraction_sg(int N, const vector<int>& moves) {
    ...
    // 变量：seen=为当前减法游戏状态标记后继 SG 值的时间戳数组 seen。
    vector<int> seen(moves.size() + 5);
    // 变量：x=当前计算 SG 值的石子数 x。
    for (int x = 1; x <= N; ++x) {
        // 用 x 作为时间戳：本轮标记只写 seen，不需要清空数组。
        // 变量：tag=复用 seen 数组时标记当前石子数 x 这一轮的时间戳 tag。
        int tag = x;
        ...
        // 变量：g=当前状态 x 的后继 SG 集合中最小未出现的非负整数 g。
        int g = 0;
        while (g < (int)seen.size() && seen[g] == tag) ++g;
        sg[x] = g;
    }
...
\begin{lstlisting}[language={C++}]
// 接口：find_period(sg, min_start, min_len)：在 SG 序列中寻找可验证周期并返回周期长度；找不到时返回 -1。
// 参数：已计算的 SG 数列 sg；允许作为周期起点的最小下标 min_start；除两段完整周期外还要验证的最少位置数 min_len。
...
// 接口：dfs_mask(mask)：返回状压状态 mask 是否为先手必胜态并记忆化；存在一个必败后继时返回 true。
// 参数：当前状压状态 mask。
bool dfs_mask(int mask) {
    // mask 的每一位代表一个“仍然可用”的对象。
    // 如果状态还有 last、资源数等信息，必须一并编码进 key。
    // 变量：it=容器 memo 中当前查找结果的迭代器 it。
    auto it = memo.find(mask);
    if (it != memo.end()) return it->second;

    // 变量：i=状压博弈状态 mask 中当前尝试选择的对象位 i。
    for (int i = 0; i < n; ++i) {
...
// 参数：当前搜索状态 s；剩余搜索深度 depth；当前已知下界 alpha；当前已知上界 beta；当前层是否轮到极大化一方 maxing。
int alpha_beta(State& s, int depth, int alpha, int beta, bool maxing) {
    ...
    if (maxing) {
        // 变量：value=极大化层当前已搜索着法中的最大局面估值 value。
        int value = INT_MIN;
        for (Move mv : moves) {
    ...
    // 变量：value=极小化层当前已搜索着法中的最小局面估值 value。
    int value = INT_MAX;
    for (Move mv : moves) {
        apply(s, mv);
