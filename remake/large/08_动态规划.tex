...
// 变量：bestEnding=必须以当前位置结尾的最大子段和 bestEnding；maxSubarray=扫描至今的最大子段和 maxSubarray。
...
// 接口：complete_knapsack(items, W)：返回每个容量上限下的完全背包最大价值；要求物品重量为正。
...
// 参数：当前 0-based/DP 位置 pos；前缀是否仍贴住上界 tight；是否已经放置非前导零数字 started；当前余数状态 rem。
...
\begin{lstlisting}[language={C++}]
// 接口：window_min_dp(base, k)：计算 dp[i]=base[i]+min(dp[j])，其中 i-k<=j<i；dp[0]=base[0]。
// 参数：每个位置的附加代价 base；正窗口宽度 k。
vector<long long> window_min_dp(const vector<long long>& base, int k) {
    // 变量：n=DP 状态总数。
    int n = (int)base.size();
    if (n == 0) return {};
    // 变量：dp=窗口最小值优化后的状态数组。
...
\begin{lstlisting}[language={C++}]
// 接口：solve(l, r, optl, optr)：完成本节给出的单组测试/递归区间；入口函数；每组测试前按注释清空全局状态。
