...
// 接口：polar_cmp(a, b)：按极角比较两个向量，供 sort 使用；a 的极角应排在 b 前时返回 true。
// 参数：第一个待比较极角的向量 a；第二个待比较极角的向量 b。
...
\begin{lstlisting}[language={C++}]
// 接口：closest(l, r)：返回 0-based 半开区间 [l,r) 内最近点对距离；返回类型 long double。
// 参数：待处理 0-based 半开区间 [l,r) 的左端点 l；待处理 0-based 半开区间 [l,r) 的右端点 r。
long double closest(int l, int r) {
    ...
    // 变量：strip=最近点对分治中靠近中线、仍可能改进答案的候选点带 strip。
    vector<Point> strip;
...
\begin{lstlisting}[language={C++}]
// 接口：circle_circle(a, r, b, R)：返回两圆全部交点（0/1/2 个；重合圆需单独处理）；返回类型 vector<Point>。
// 参数：第一个输入圆的圆心 a；第一个圆的非负半径 r；第二个输入圆的圆心 b；第二个圆的非负半径 R。
vector<Point> circle_circle(Point a, long double r,
    ...
    // 变量：e=从圆心 a 指向圆心 b 的单位方向向量 e；base=两圆公共弦中点 base。
    Point e = d * (1 / len), base = a + e * x;
    // 变量：per=与圆心连线单位方向 e 垂直的单位向量 per。
    Point per = {-e.y, e.x};

    // h2=0 是相切；否则关于圆心连线对称地产生两个交点。
    if (h2 < EPS) return {base};
    // 变量：h=圆交点相对圆心连线的垂直偏移长度 h。
...
\begin{lstlisting}[language={C++}]
// 接口：line_circle(p, v, o, r)：返回直线与圆的全部交点（0/1/2 个）；返回类型 vector<Point>。
// 参数：直线经过的基点 p；直线方向向量 v；圆心 o；圆的非负半径 r。
vector<Point> line_circle(Point p, Point v, Point o, long double r) {
    // 直线参数式：P(t)=p+t*v；如果题目给的是线段，最后还要检查 t in [0,1]。
    // 变量：q=从圆心 o 指向直线基点 p 的位移向量 q。
    Point q = p - o;
    // 变量：A=直线参数方程代入圆方程后的二次项系数 A；B=直线参数方程代入圆方程后的一次项系数 B。
    long double A = dot(v, v), B = 2 * dot(q, v);
    // 变量：C=直线参数方程代入圆方程后的常数项 C。
    long double C = dot(q, q) - r * r;
    // 变量：D=直线与圆求交二次方程的判别式 D。
...
struct CoverNode {
    ...
    __int128 len = 0;  // 该节点代表范围内至少被覆盖一次的精确长度。
};
// 变量：xs=离散化后排序去重的坐标 xs。
vector<long long> xs;
// 变量：seg=扫描线维护的线段树 seg。
CoverNode seg[MAXN * 4];
// 接口：pull(p, l, r)：按覆盖计数和两个孩子重算扫描线节点 p 表示的实际覆盖长度；会修改当前结构；多测时重新构造或显式清空成员。
// 参数：当前覆盖长度线段树节点编号 p（根为 1）；节点 p 表示的离散下标半开区间 [l,r) 左端点 l；节点 p 表示的离散下标半开区间 [l,r) 右端点 r。
void pull(int p, int l, int r) {
    // xs[l..r] 是坐标点，节点覆盖的是 [xs[l], xs[r])。
    if (seg[p].tag) seg[p].len = (__int128)xs[r] - xs[l];
    else if (r - l == 1) seg[p].len = 0;
    else seg[p].len = seg[p<<1].len + seg[p<<1|1].len;
}
// 接口：modify(p, l, r, ql, qr, d)：给扫描线离散下标半开区间 [ql,qr) 增加覆盖计数 d；会修改当前结构；多测时重新构造或显式清空成员。
// 参数：当前覆盖长度线段树节点编号 p（根为 1）；节点 p 表示的离散下标半开区间 [l,r) 左端点 l；节点 p 表示的离散下标半开区间 [l,r) 右端点 r；目标离散下标半开区间 [ql,qr) 左端点 ql；目标离散下标半开区间 [ql,qr) 右端点 qr；本次加入或移除的覆盖计数增量 d。
...
// 接口：union_area(e)：矩形并面积可由 long long 表示时返回该值，否则返回 nullopt；空事件返回 0。
// 参数：矩形横边事件 e；xs 需预先收集所有横坐标、排序并去重。
optional<long long> union_area(vector<Event> e) {
    if (e.empty() || xs.size() < 2) return 0;
    // 事件按 y 扫描；在处理当前 y 前，seg[1].len 对应上一段高度。
    // 接口：比较器 lambda(a, b)：按矩形扫描线事件的 y 坐标升序排列。
    // 参数：第一个扫描线事件 a；第二个扫描线事件 b。
    sort(e.begin(), e.end(), [](auto& a, auto& b) { return a.y < b.y; });
    // 变量：ans=扫描线按高度条带累计的矩形并面积 ans；last_y=扫描线上一个事件纵坐标 last_y。
    __int128 ans = 0, last_y = e[0].y;
    // 变量：max_area=接口允许返回的最大面积。
    const __int128 max_area = numeric_limits<long long>::max();
    // 变量：i=扫描线中当前 y 坐标事件组的首事件下标 i。
    for (int i = 0; i < (int)e.size();) {
        // 先按 long long 返回上限做除法检查，避免极端坐标的宽×高本身溢出 __int128。
        // 变量：height=当前事件与上一批事件之间的非负扫描高度。
        __int128 height = (__int128)e[i].y - last_y;
        if (seg[1].len != 0 && height > max_area / seg[1].len)
            return nullopt;
        // 变量：delta_area=本段新增且已确认不超过 long long 上限的面积。
        __int128 delta_area = seg[1].len * height;
        if (delta_area > max_area - ans) return nullopt;
        ans += delta_area;
        // 变量：j=扫描线中从 i 开始寻找同一 y 坐标事件组末尾的下标 j。
        int j = i;
        while (j < (int)e.size() && e[j].y == e[i].y) {
            // 同一条扫描线上的事件必须先全部更新，再进入下一段高度。
            // 变量：l=扫描线事件左端横坐标 x1 的离散下标 l。
            int l = lower_bound(xs.begin(), xs.end(), e[j].x1) - xs.begin();
            // 变量：r=扫描线事件右端横坐标 x2 的离散下标 r；更新半开区间 [l,r)。
