...
// 参数：当前线段树节点编号 p（根通常为 1）；合法闭区间 [l,r]；0<MOD 且 sum、mul、add 与已有标记均规范化到 [0,MOD)。
...
// 接口：max_xor(left_root, right_root, bit, x)：返回 x 与区间内元素的最大异或值；返回类型 unsigned long long。
...
struct Line {
    ...
    // 接口：get(x)：在 __int128 中返回直线在横坐标 x 处的精确值；空直线返回宽整数负无穷。
    // 参数：待处理值或点/状态 x。
