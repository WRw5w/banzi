...
单词/前缀计数 & Trie & \passthrough{\lstinline!O(L)!} \\
...
vector<int> prefix_function(const string& s) {
    ...
    // 变量：i=前缀函数正在计算 pi[i] 的字符串位置 i。
    for (int i = 1; i < (int)s.size(); ++i) {
        // 变量：j=前缀函数计算中当前可回退的已匹配前缀长度 j。
        int j = pi[i - 1];
...
vector<int> z_function(const string& s) {
    // 变量：n=容器 s 的元素数量 n；l=当前区间左端点 l；r=当前区间右端点 r；z=Z 函数数组 z；z[i] 是 s 与 s[i..] 的最长公共前缀。
    int n = s.size(), l = 0, r = 0; vector<int> z(n);
    if (n) z[0] = n;
    // 变量：i=Z 函数正在计算 z[i] 的后缀起点 i。
    for (int i = 1; i < n; ++i) {
...
// 变量：h=字符串哈希的前缀哈希数组 h；pw=幂次预处理数组 pw。
vector<unsigned long long> h(n + 1), pw(n + 1, 1);
// 变量：i=字符串哈希正在扩展前缀哈希的字符位置 i。
for (int i = 0; i < n; ++i)
    h[i + 1] = h[i] * BASE + (unsigned char)s[i], pw[i + 1] = pw[i] * BASE;
// 接口：get_hash(l, r)：返回字符串 0-based 半开区间 [l,r) 的哈希值。
// 参数：子串左端点 l；子串右端点 r（不含）。
...
// 变量：i=Manacher 扩展串中当前计算回文半径的中心位置 i。
...
\subsection{普通字符串字典树（Trie）}\label{ux666eux901aux5b57ux7b26ux4e32ux5b57ux5178ux6811trie}

字符集固定时用数组保存儿子。下面的模板只接受小写字母；\passthrough{\lstinline!terminal[u]!} 统计完整字符串，\passthrough{\lstinline!pass[u]!} 统计以当前路径为前缀的字符串。重复插入会重复计数，单次插入或查询复杂度均为 \passthrough{\lstinline!O(L)!}。节点上限至少取所有不同前缀数加一，大数组对象应放在全局区；多测时重新构造或显式清空。

\begin{lstlisting}[language={C++}]
struct StringTrie {
    int ch[MAXN][26]{}; // ch[u][c]=节点 u 沿字符 c 的儿子；0 表示不存在。
    int pass[MAXN]{};   // pass[u]=经过节点 u 的已插入字符串数量。
    int terminal[MAXN]{}; // terminal[u]=恰好在节点 u 结尾的字符串数量。
    int tot = 0;        // 当前最后一个节点编号；根为 0，节点范围 [0,tot]。

    // 接口：insert(s)：插入一个仅含小写字母的字符串；重复字符串会重复计数。
    // 参数：待插入的字符串 s。
    ...
    // 接口：count_word(s)：返回字符串 s 被插入的次数；不存在时返回 0。
    // 参数：待查询的完整字符串 s。
    ...
    // 接口：count_prefix(s)：返回以字符串 s 为前缀的已插入字符串数量。
    // 参数：待查询的前缀字符串 s；空串前缀返回已插入字符串总数。
...
\subsection{Aho--Corasick 多模式匹配自动机}\label{ahocorasick-ux591aux6a21ux5f0fux5339ux914dux81eaux52a8ux673a}

AC 自动机先把模式串插入 Trie，再用 BFS 建 fail：节点的 fail 指向最长真后缀状态，匹配时沿 fail 转移；把输出计数沿 fail 累加可统计所有模式出现次数。模式集合固定后一次建树、多次查询。
...
struct AC {
    ...
    // 接口：query(s)：扫描主串 s 并返回全部模式串出现次数之和；返回类型 int。
    // 参数：输入字符串 s。
    int query(const string& s) {
        // 变量：u=字符串自动机从根开始扫描的状态编号 u；ans=AC 自动机扫描主串时累计的全部模式匹配次数 ans。
        int u = 0, ans = 0;
        // 变量：c=从容器 s 枚举的元素 c。
        for (char c : s) {
...
vector<int> suffix_array(string s) {
    ...
    // 变量：sa=后缀数组 sa；sa[i] 是第 i 小后缀起点；r=当前轮每个后缀长度 k 前缀的等价类排名数组 r；nr=按长度 2k 重新排序后得到的新排名数组 nr。
    ...
    for (int k = 1;; k <<= 1) {
        ...
        sort(sa.begin(), sa.end(), [&](int i, int j) {
            ...
            // 变量：ri=后缀 i 偏移 k 后第二段的等价类排名 ri；越界时为 -1。
            int ri = i + k < n ? r[i + k] : -1;
            // 变量：rj=后缀 j 偏移 k 后第二段的等价类排名 rj；越界时为 -1。
...
struct SAM {
    ...
    void extend(char c) {
        ...
        // 变量：cur=加入新字符后创建的 SAM 状态编号 cur；p=SAM 扩展前原末状态的游走指针 p。
...
struct PAM {
    ...
    // 接口：add(c)：向回文自动机追加字符，更新 last 并返回新的最长回文后缀状态编号；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：待加入自动机的字符 c。
    void add(char c) {
        // 先把新字符写入串，再寻找能够扩展的最长回文后缀。
        // 变量：u=沿 fail 链找到的可被当前字符扩展的回文状态 u。
        s[++n] = c - 'a'; int u = get_fail(last);
        if (!next[u][s[n]]) {
            // 这是一个此前未出现过的新回文节点。
            // 变量：v=本次新建的回文自动机状态编号 v。
    ...
    // 接口：build(str)：清空回文自动机，依次加入 str 的字符并沿 fail 汇总出现次数；完成初始化；多测时每组重新调用。
    // 参数：待构建自动机的完整字符串 str。
    int build(const string& str) {
        init();
        // 变量：c=从容器 str 枚举的元素 c。
        for (char c : str) add(c);
        // fail 从长回文指向短回文，必须按长度降序汇总；
        // 许多简化板子用编号倒序，只在编号也随长度单调时才安全。
        // 变量：ord=按回文长度从大到小排列、用于沿 fail 链汇总出现次数的回文自动机状态编号 ord。
...
int min_rotation(const string& s) {
    // 变量：t=用于寻找最小循环表示的双倍字符串 t=s+s；i=最小循环表示算法当前第一个候选起点 i；j=最小循环表示算法当前第二个候选起点 j；n=容器 s 的元素数量 n。
    string t = s + s; int i = 0, j = 1, n = s.size();
    while (i < n && j < n) {
        // 变量：k=比较两个循环表示时已匹配的偏移长度 k。
        int k = 0;
