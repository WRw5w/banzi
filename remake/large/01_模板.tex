...
// 接口：print_i128(x)：十进制输出一个 __int128；不返回值。
// 参数：待十进制输出的 __int128 整数 x。
void print_i128(i128 x) { // 需要输出超出 long long 时使用
    // 先转成无符号绝对值，避免对 INT128_MIN 直接取反产生有符号溢出。
    // 变量：value=x 的无符号绝对值，可完整容纳 2^127。
    u128 value = static_cast<u128>(x);
    if (x < 0) { cout << '-'; value = 0 - value; }
    // 变量：s=从标准输入读取、供当前字符串算法处理的字符串 s。
    string s;
    do { s.push_back('0' + value % 10); value /= 10; } while (value);
