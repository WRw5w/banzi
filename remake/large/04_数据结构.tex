...
struct LiChao {
    ...
    struct Line {
        ...
        // 接口：get(x)：在 __int128 中返回直线在横坐标 x 处的精确值；空直线返回宽整数正无穷。
        // 参数：待处理值或点/状态 x。
  ...
  // 参数：闭区间左端点 L；闭区间右端点 R。
    ...
    // 接口：query(x, p, l, r)：返回本节数据结构在给定位置/区间上的精确查询结果；返回类型 __int128。
    ...
    // 接口：query(x)：返回本节数据结构在给定位置/区间上的精确查询结果；返回类型 __int128。
    // 参数：待处理值或点/状态 x。
...
struct SegBeats {
  ...
  // 参数：输入点/向量 a。
    ...
    // 接口：query_sum(p, l, r, ql, qr)：返回闭区间查询的精确元素和；返回类型 __int128。
