\chapter{数据结构}\label{ux6570ux636eux7ed3ux6784}

\section{STL 必会组件}\label{stl-ux5fc5ux4f1aux7ec4ux4ef6}

\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
需求 & 容器/算法 & 复杂度 \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
动态数组 & \passthrough{\lstinline!vector!} & 末尾均摊 \passthrough{\lstinline!O(1)!}，随机访问 \passthrough{\lstinline!O(1)!} \\
双端队列 & \passthrough{\lstinline!deque!} & 两端 \passthrough{\lstinline!O(1)!} \\
栈/队列 & \passthrough{\lstinline!stack!} / \passthrough{\lstinline!queue!} & \passthrough{\lstinline!O(1)!} \\
优先队列 & \passthrough{\lstinline!priority\_queue!} & 插入/删除顶 \passthrough{\lstinline!O(log n)!} \\
有序集合 & \passthrough{\lstinline!set!} / \passthrough{\lstinline!multiset!} & 增删查 \passthrough{\lstinline!O(log n)!} \\
有序键值 & \passthrough{\lstinline!map!} & \passthrough{\lstinline!O(log n)!} \\
哈希键值 & \passthrough{\lstinline!unordered\_map!} & 期望 \passthrough{\lstinline!O(1)!}，最坏可能退化 \\
去重/计数 & \passthrough{\lstinline!sort!} + \passthrough{\lstinline!unique!} / \passthrough{\lstinline!map!} & \passthrough{\lstinline!O(n log n)!} \\
\end{longtable}

\passthrough{\lstinline!set!} 的 \passthrough{\lstinline!lower\_bound!}、\passthrough{\lstinline!priority\_queue<pair<...>>!} 的字典序比较必须熟练。

\passthrough{\lstinline!multiset::erase(iterator)!} 与 \passthrough{\lstinline!erase(value)!} 的差别也要注意。

\begin{lstlisting}[language={C++}]
// 变量：a=输入数组 input 的工作副本 a。
vector<int> a = input;
sort(a.begin(), a.end());
a.erase(unique(a.begin(), a.end()), a.end());
// 变量：pq=当前优先队列 pq；堆顶含义见比较器。
priority_queue<pair<int, int>> pq;
// 变量：i=在线中位数初始化时加入优先队列的数组位置 i。
for (int i = 0; i < n; ++i) pq.push({value[i], i});
while (!pq.empty()) {
    auto [value, id] = pq.top(); pq.pop();
    // 处理当前最大值；必要时用 version[id] 判断是否过期
}
\end{lstlisting}

\section{并查集（Disjoint Set Union，DSU）}\label{ux5e76ux67e5ux96c6disjoint-set-uniondsu}

维护若干不相交集合，支持合并和查询连通性。路径压缩 + 按大小合并后，单次摊销 \passthrough{\lstinline!O(α(n))!}。

可扩展：维护集合大小、权值和、到父亲的距离/奇偶、撤销栈。带权并查集常用于维护 \passthrough{\lstinline!dist[x]-dist[y]=w!} 或敌友关系。

\begin{lstlisting}[language={C++}]
struct DSU {
    vector<int> p, sz; // p[x]=父节点；sz[root]=该集合大小；点编号 1..n。
  // 接口：DSU(n)：构造管理点 1..n 的并查集，每个点初始自成集合；构造并初始化对象；每组测试建议重新构造。
  // 参数：并查集管理的元素编号上界 n。
    DSU(int n): p(n + 1), sz(n + 1, 1) { iota(p.begin(), p.end(), 0); }
    // 接口：find(x)：返回 x 当前所在并查集的代表元；返回类型 int。
    // 参数：待查询代表元的元素编号 x。
    int find(int x) { return p[x] == x ? x : p[x] = find(p[x]); }
    // 接口：unite(a, b)：合并两个元素所在集合；原本同集合时返回 false；实际合并两个集合时返回 true。
    // 参数：第一个待合并元素编号 a；第二个待合并元素编号 b。
    bool unite(int a, int b) {
        a = find(a); b = find(b); if (a == b) return false;
        if (sz[a] < sz[b]) swap(a, b);
        p[b] = a; sz[a] += sz[b]; return true;
    }
};
\end{lstlisting}

Kruskal、离线加边、连通块统计、最小生成树、带限制的图连通性都优先考虑 DSU。

\section{树状数组（Fenwick Tree）}\label{ux6811ux72b6ux6570ux7ec4fenwick-tree}

支持单点加、前缀和、区间和，\passthrough{\lstinline!O(log n)!}；空间 \passthrough{\lstinline!O(n)!}。通过差分可支持区间加/单点查；两棵树可支持区间加/区间和。下标必须从 1 开始。

\begin{lstlisting}[language={C++}]
struct BIT {
    int n;                 // 可访问下标为 1..n；禁止用 0 调 add。
    vector<long long> t;   // Fenwick 内部树状数组，不是原数组值。
  // 接口：BIT(n)：构造管理 1..n 下标的空树状数组；构造并初始化对象；每组测试建议重新构造。
  // 参数：树状数组管理的最大下标 n。
    BIT(int n): n(n), t(n + 1) {}
    // 接口：add(x, v)：把增量 v 加到树状数组的 1-based 位置 x；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：树状数组的 1-based 修改位置 x；位置 x 的点增加量 v。
    void add(int x, long long v) { for (; x <= n; x += x & -x) t[x] += v; }
    // 接口：sum(x)：返回树状数组闭前缀 [1,x] 内已存数值的总和；返回类型 long long。
    // 参数：树状数组前缀查询的 1-based 右端点 x。
    // 变量：r=树状数组前缀 [1,x] 的累计和 r。
    long long sum(int x) const { long long r = 0; for (; x; x -= x & -x) r += t[x]; return r; }
    // 接口：range(l, r)：返回 1-based 闭区间 [l,r] 的元素和；返回类型 long long。
    // 参数：闭区间左端点 l；闭区间右端点 r。
    long long range(int l, int r) const { return sum(r) - sum(l - 1); }
};
\end{lstlisting}

应用：逆序对、动态排名、离线矩形计数、二维偏序（配合 CDQ/排序）、第 k 个值（二进制提升）。

\section{线段树}\label{ux7ebfux6bb5ux6811}

支持任意可合并区间信息。节点存区间答案，\passthrough{\lstinline!push\_up!} 合并左右儿子；区间修改要有懒标记并在访问子节点前 \passthrough{\lstinline!push\_down!}。

\subsection{常见维护}\label{ux5e38ux89c1ux7ef4ux62a4}

\begin{itemize}
\tightlist
\item
  区间和 + 区间加：标记加法，子节点和增加 \passthrough{\lstinline!tag * len!}。
\item
  区间和 + 区间乘/加：标记为仿射变换 \passthrough{\lstinline!x -> mul*x+add!}，下传时 \passthrough{\lstinline!add = add*mul\_parent + add\_parent!}。
\item
  区间最值 + 区间加：维护最大/最小值及懒加。
\item
  区间赋值：赋值标记覆盖旧加法；标记优先级必须明确。
\item
  扫描线：维护每个离散小区间是否被覆盖，保存 \passthrough{\lstinline!cover!} 与 \passthrough{\lstinline!len!}。
\end{itemize}

复杂度：建树 \passthrough{\lstinline!O(n)!}，单次查询/修改 \passthrough{\lstinline!O(log n)!}。只要操作满足结合律，可把节点信息设计成结构体。

\begin{lstlisting}[language={C++}]
struct SegTree {
    int n;                    // 原数组下标为闭区间 [1,n]。
    vector<long long> sum;    // sum[p]=节点 p 所管区间的元素和。
    vector<long long> lazy;   // lazy[p]=尚未下传的“区间每项增加量”。
  // 接口：SegTree(n)：构造管理 1..n 闭区间的空线段树；构造并初始化对象；每组测试建议重新构造。
  // 参数：线段树管理的 1-based 数组长度及最大下标 n。
    SegTree(int n): n(n), sum(4 * n + 4), lazy(4 * n + 4) {}
    // 接口：apply(p, l, r, v)：把本次懒操作直接作用于节点 p，并同步节点值与标记；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：当前线段树节点编号 p（根通常为 1）；闭区间左端点 l；闭区间右端点 r；目标区间内每个元素的统一增加量 v。
    void apply(int p, int l, int r, long long v) {
        sum[p] += v * (r - l + 1); lazy[p] += v;
    }
    // 接口：push(p, l, r)：把线段树节点 p 的懒标记下传到左右孩子并清空本节点标记；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：当前线段树节点编号 p（根通常为 1）；闭区间左端点 l；闭区间右端点 r。
    void push(int p, int l, int r) {
        if (!lazy[p] || l == r) return;
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        int m = (l + r) / 2;
        apply(p * 2, l, m, lazy[p]);
        apply(p * 2 + 1, m + 1, r, lazy[p]);
        lazy[p] = 0;
    }
    // 接口：add(p, l, r, ql, qr, v)：把目标闭区间 [ql,qr] 的元素统一增加 v；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：当前线段树节点编号 p（根通常为 1）；闭区间左端点 l；闭区间右端点 r；目标闭区间左端 ql；目标闭区间右端 qr；目标区间内每个元素的统一增加量 v。
    void add(int p, int l, int r, int ql, int qr, long long v) {
        if (ql <= l && r <= qr) return apply(p, l, r, v);
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        push(p, l, r); int m = (l + r) / 2;
        if (ql <= m) add(p * 2, l, m, ql, qr, v);
        if (qr > m) add(p * 2 + 1, m + 1, r, ql, qr, v);
        sum[p] = sum[p * 2] + sum[p * 2 + 1];
    }
    // 接口：query(p, l, r, ql, qr)：返回目标闭区间 [ql,qr] 的元素和；返回类型 long long。
    // 参数：当前线段树节点编号 p（根通常为 1）；闭区间左端点 l；闭区间右端点 r；目标闭区间左端 ql；目标闭区间右端 qr。
    long long query(int p, int l, int r, int ql, int qr) {
        if (ql <= l && r <= qr) return sum[p];
        // 变量：m=当前递归区间 [l,r] 的中点 m；ans=线段树查询目标区间时从左右子树累加的区间和 ans。
        push(p, l, r); int m = (l + r) / 2; long long ans = 0;
        if (ql <= m) ans += query(p * 2, l, m, ql, qr);
        if (qr > m) ans += query(p * 2 + 1, m + 1, r, ql, qr);
        return ans;
    }
};
\end{lstlisting}

\subsection{线段树上的二分}\label{ux7ebfux6bb5ux6811ux4e0aux7684ux4e8cux5206}

维护区间和/最大值时，可从根向下找``第一个满足条件的位置''，每层只走一个孩子，\passthrough{\lstinline!O(log n)!}，比外层二分 + 查询更快。

\begin{lstlisting}[language={C++}]
// 接口：first_at_least(p, l, r, x)：返回查询范围内第一个值至少为 x 的位置，不存在返回 -1；返回类型 int。
// 参数：当前线段树节点编号 p（根通常为 1）；闭区间左端点 l；闭区间右端点 r；查询要求达到的值下界 x。
int first_at_least(int p, int l, int r, long long x) {
    if (tree[p] < x) return -1;
    if (l == r) return l;
    // 变量：m=当前递归区间 [l,r] 的中点 m。
    int m = (l + r) / 2;
    if (tree[p * 2] >= x) return first_at_least(p * 2, l, m, x);
    return first_at_least(p * 2 + 1, m + 1, r, x);
}
\end{lstlisting}

\section{ST 表（Sparse Table，静态区间查询）}\label{ux7a00ux758fux8868sparse-tablest}

静态数组的幂等运算（min/max/gcd/and/or）预处理 \passthrough{\lstinline!O(n log n)!}，查询 \passthrough{\lstinline!O(1)!}：

\passthrough{\lstinline!k=floor(log2(len))!}，答案为 \passthrough{\lstinline!merge(st[k][l], st[k][r-2\^k+1])!}。不可用于 sum 这类非幂等运算（重叠会重复计数）。
下面的实现约定输入数组非空，数组下标和查询端点均为 \passthrough{\lstinline!0-based!}，查询区间为闭区间 \passthrough{\lstinline![l,r]!}。

\begin{lstlisting}[language={C++}]
struct SparseTable {
    // 变量：st[k][i]=从 i 开始、长度为 2^k 的区间最小值。
    vector<vector<int>> st;
    // 变量：lg[len]=floor(log2(len))，用于确定查询所需层数。
    vector<int> lg;

    // 接口：SparseTable(a)：用非空数组 a 构建区间最小值 ST 表；预处理 O(n log n)。
    explicit SparseTable(const vector<int>& a) {
        // 变量：n=容器 a 的元素数量 n。
        int n = a.size();
        lg.assign(n + 1, 0);
        // 变量：i=当前计算二进制对数的区间长度。
        for (int i = 2; i <= n; ++i)
            lg[i] = lg[i / 2] + 1;

        st.assign(lg[n] + 1, vector<int>(n));
        st[0] = a;
        // 变量：k=当前预处理区间长度的二进制指数。
        for (int k = 1; k < (int)st.size(); ++k)
            // 变量：i=稀疏表第 k 层当前预处理区间的 0-based 左端点 i。
            for (int i = 0; i + (1 << k) <= n; ++i)
                st[k][i] = min(st[k - 1][i],
                               st[k - 1][i + (1 << (k - 1))]);
    }

    // 接口：query(l,r)：返回 0-based 闭区间 [l,r] 的最小值；要求 0<=l<=r<n。
    int query(int l, int r) const {
        // 变量：k=floor(log2(r-l+1))，两段长度 2^k 的区间可覆盖查询范围。
        int k = lg[r - l + 1];
        return min(st[k][l], st[k][r - (1 << k) + 1]);
    }
};
\end{lstlisting}

\section{堆与可并堆思想}\label{ux5806ux4e0eux53efux5e76ux5806ux601dux60f3}

\passthrough{\lstinline!priority\_queue!} 用于每步取最小/最大。双堆维护中位数（小根维护大半边，大根维护小半边）。懒删除：堆中保留旧值，弹出时与当前版本/计数比对。

左偏树/可并堆支持两个堆合并，合并复杂度 \passthrough{\lstinline!O(log n)!}；题目要求``合并集合并取最值''时考虑。斜堆实现更短但无严格最坏保证。

\begin{lstlisting}[language={C++}]
priority_queue<int> low; // 小的一半，堆顶是最大值
// 变量：high=双堆中保存较大一半元素的小根堆 high。
priority_queue<int, vector<int>, greater<int>> high;
// 接口：add_number(x)：把整数 x 加入双堆中位数结构并重新平衡两堆大小；不返回值。
// 参数：待加入统计结构的整数 x。
void add_number(int x) {
    if (low.empty() || x <= low.top()) low.push(x); else high.push(x);
    if (low.size() > high.size() + 1) high.push(low.top()), low.pop();
    if (high.size() > low.size()) low.push(high.top()), high.pop();
}
// 接口：median()：返回当前两个堆维护的中位数（奇数个元素时为正中间）；返回类型 int。
int median() { return low.top(); }
\end{lstlisting}

\section{单调结构与差分约束}\label{ux5355ux8c03ux7ed3ux6784ux4e0eux5deeux5206ux7ea6ux675f}

单调栈/队列见 \href{01-基础与实现规范.md}{01-基础与实现规范}。差分约束
\passthrough{\lstinline!x[v] <= x[u] + w!} 建边 \passthrough{\lstinline!u->v!}；
把全部点作为源点可检查任意连通分量中的负环。

\begin{lstlisting}[language={C++}]
// 接口：difference_constraints(n, edges)：在 __int128 域求一组满足 x[v]<=x[u]+w 的势函数；存在负环时返回 nullopt。
// 参数：变量数 n；约束边 (u,v,w)，顶点使用 0-based 编号。
optional<vector<__int128>> difference_constraints(
        int n, const vector<tuple<int, int, long long>>& edges) {
    // 变量：g=由差分约束构成的有向带权邻接表。
    vector<vector<pair<int, long long>>> g(n);
    // 变量：u=约束边起点；v=约束边终点；w=x[v]-x[u] 的上界。
    for (auto [u, v, w] : edges) g[u].push_back({v, w});
    // 变量：dist=当前找到的各变量可行上界势函数。
    vector<__int128> dist(n, 0);
    // 变量：path_edges=产生当前 dist 值的松弛路径边数，用于识别负环。
    vector<int> path_edges(n, 0);
    // 变量：in_queue=顶点当前是否已经在待松弛队列中。
    vector<char> in_queue(n, true);
    // 变量：q=等待扫描出边的顶点队列。
    queue<int> q;
    // 变量：u=初始化入队的顶点编号。
    for (int u = 0; u < n; ++u) q.push(u);
    while (!q.empty()) {
        // 变量：u=本轮从队首取出的约束边起点。
        int u = q.front(); q.pop(); in_queue[u] = false;
        // 变量：v=约束边终点；w=x[v]-x[u] 的上界。
        for (auto [v, w] : g[u]) {
            // 变量：candidate=经约束边 u->v 得到的宽整数候选势函数值。
            __int128 candidate = dist[u] + (__int128)w;
            if (dist[v] <= candidate) continue;
            dist[v] = candidate;
            path_edges[v] = path_edges[u] + 1;
            if (path_edges[v] >= n) return nullopt;
            if (!in_queue[v]) in_queue[v] = true, q.push(v);
        }
    }
    return dist;
}
\end{lstlisting}

\section{字典树（Trie，Prefix Tree）}\label{ux5b57ux5178ux6811trieprefix-tree}

字符集固定时数组儿子最快；动态字符集用 \passthrough{\lstinline!map!}。插入/查询长度 \passthrough{\lstinline!L!} 的字符串均为 \passthrough{\lstinline!O(L)!}。二进制 Trie 支持最大异或、区间内最大异或（结合持久化或离线前缀）。

\begin{lstlisting}[language={C++}]
struct BinaryTrie {
    int ch[200000][2]{}; // ch[u][bit]=下一节点编号；0 表示不存在。
    int cnt[200000]{};   // cnt[u]=经过节点 u 的已插入数字个数。
    int nodes = 1;       // 已分配节点数；根节点固定为 0。
    // 接口：insert(x)：把整数 x 的各二进制位插入 Trie；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：待按二进制位插入 Trie 的非负整数 x。
    void insert(int x) {
        // 变量：u=二进制 Trie 根节点编号 u。
        int u = 1; ++cnt[u];
        // 变量：b=二进制 Trie 当前处理的位编号 b。
        for (int b = 30; b >= 0; --b) {
            // 变量：bit=二进制 Trie 插入时从高到低处理的位编号 bit。
            int bit = (x >> b) & 1;
            if (!ch[u][bit]) ch[u][bit] = ++nodes;
            u = ch[u][bit]; ++cnt[u];
        }
    }
    // 接口：max_xor(x)：沿二进制 Trie 贪心返回与 x 的最大异或值；返回类型 int。
    // 参数：求最大异或值时使用的初始值 x。
    int max_xor(int x) {
        // 变量：u=二进制 Trie 根节点编号 u；ans=二进制 Trie 贪心过程中已经确定的最大异或值 ans。
        int u = 1, ans = 0;
        // 变量：b=二进制 Trie 当前处理的位编号 b。
        for (int b = 30; b >= 0; --b) {
            // 变量：bit=二进制 Trie 最大异或查询中从高到低处理的位编号 bit。
            int bit = (x >> b) & 1;
            if (ch[u][bit ^ 1] && cnt[ch[u][bit ^ 1]])
                ans |= 1 << b, u = ch[u][bit ^ 1];
            else u = ch[u][bit];
        }
        return ans;
    }
};
\end{lstlisting}

\section{可持久化数据结构}\label{ux53efux6301ux4e45ux5316ux6570ux636eux7ed3ux6784}

每次修改只复制从根到叶子的 \passthrough{\lstinline!O(log n)!} 个节点，旧版本仍可查询。主席树（可持久化权值线段树）用于静态区间第 k 小：\passthrough{\lstinline!root[r] - root[l-1]!} 在树上同步下行，\passthrough{\lstinline!O(log V)!}。注意节点池容量约为 \passthrough{\lstinline!版本数 × log V!}。

\begin{lstlisting}[language={C++}]
struct Node { int l = 0, r = 0, sum = 0; } tr[4000000]; // 左右儿子编号及该值域节点计数。
// 变量：tot=当前已经创建的数据结构节点数量 tot。
int tot = 0;
// 接口：update(old, l, r, pos)：从旧版本根 old 复制到 pos 的路径、把该值域计数加一并返回新版本根；返回类型 int。
// 参数：旧版本根节点编号 old；闭区间左端点 l；闭区间右端点 r；持久化线段树中要修改的 0-based 位置 pos。
int update(int old, int l, int r, int pos) {
    // 变量：now=本次路径复制新建的持久化权值线段树节点编号 now。
    int now = ++tot; tr[now] = tr[old]; ++tr[now].sum;
    if (l != r) {
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        int m = (l + r) / 2;
        if (pos <= m) tr[now].l = update(tr[old].l, l, m, pos);
        else tr[now].r = update(tr[old].r, m + 1, r, pos);
    }
    return now;
}
// 接口：kth(u, v, l, r, k)：返回前缀版本差 v-u 所表示区间内第 k 小元素的离散值下标；返回类型 int。
// 参数：左端前缀版本根 u；右端前缀版本根 v；闭区间左端点 l；闭区间右端点 r；查询的 1-based 排名 k。
int kth(int u, int v, int l, int r, int k) {
    if (l == r) return l;
    // 变量：left_count=当前节点左子树中的元素数量 left_count。
    int left_count = tr[tr[v].l].sum - tr[tr[u].l].sum;
    // 变量：m=当前递归区间 [l,r] 的中点 m。
    int m = (l + r) / 2;
    if (k <= left_count) return kth(tr[u].l, tr[v].l, l, m, k);
    return kth(tr[u].r, tr[v].r, m + 1, r, k - left_count);
}
\end{lstlisting}

\section{分块与莫队算法（Mo's Algorithm）}\label{ux5206ux5757ux4e0eux83abux961fux7b97ux6cd5mos-algorithm}

\subsection{块状数组}\label{ux5757ux72b6ux6570ux7ec4}

把数组分成 \passthrough{\lstinline!√n!} 块，块内暴力、块间整块处理；区间加/区间和约 \passthrough{\lstinline!O(√n)!}。适合操作复杂、难以线段树维护的信息。

\begin{lstlisting}[language={C++}]
// 变量：B=块状数组采用的块长 B。
int B = max(1, (int)sqrt(n));
// 变量：a=块状数组逐点保存的原始元素数组 a；tag=当前节点保存的懒标记 tag。
vector<long long> a(n), tag((n + B - 1) / B);
// 接口：add(l, r, v)：用分块懒标记把闭区间 [l,r] 的元素统一增加 v；会修改当前结构；多测时重新构造或显式清空成员。
// 参数：闭区间左端点 l；闭区间右端点 r；闭区间 [l,r] 内每个元素的统一增加量 v。
void add(int l, int r, long long v) {
    // 变量：bl=当前块左端编号 bl；br=当前块右端编号 br。
    int bl = l / B, br = r / B;
    // 变量：i=分块区间修改中直接更新的数组位置 i。
    if (bl == br) { for (int i = l; i <= r; ++i) a[i] += v; return; }
    // 变量：i=分块修改左侧不完整块中直接更新的数组位置 i。
    for (int i = l; i < (bl + 1) * B; ++i) a[i] += v;
    // 变量：b=块状数组当前整块更新的块编号 b。
    for (int b = bl + 1; b < br; ++b) tag[b] += v;
    // 变量：i=分块修改右侧不完整块中直接更新的数组位置 i。
    for (int i = br * B; i <= r; ++i) a[i] += v;
}
\end{lstlisting}

\subsection{莫队}\label{ux83abux961f}

离线排序询问的左右端点，维护当前区间并增删元素；移动总复杂度约 \passthrough{\lstinline!O((n+q)√n)!}。需要可逆的 \passthrough{\lstinline!add/remove!}，如频次、不同数个数、答案贡献。带修改莫队多一个时间维，块大小约 \passthrough{\lstinline!n\^(2/3)!}。

\begin{lstlisting}[language={C++}]
// 变量：block=普通莫队按查询左端点分块时使用的块长 block。
int block;
struct Query { int l, r, id; }; // 1-based 闭区间 [l,r] 与原始询问编号 id。
// 接口：比较器 lambda(x, y)：按莫队左端块排序，并在同块内按奇偶交替排列右端点。
// 参数：第一个莫队查询 x；第二个莫队查询 y。
sort(qs.begin(), qs.end(), [&](const Query& x, const Query& y) {
    // 变量：bx=查询 x 所在块编号 bx；by=查询 y 所在块编号 by。
    int bx = x.l / block, by = y.l / block;
    if (bx != by) return bx < by;
    return (bx & 1) ? x.r > y.r : x.r < y.r;
});
// 变量：L=普通莫队当前 1-based 窗口的左端点 L；R=普通莫队当前 1-based 窗口的右端点 R；空窗口时 R=0；distinct=当前窗口内不同值的数量 distinct。
int L = 1, R = 0, distinct = 0; // 1-based 空窗口 [1,0]。
// 接口：add(p)：把位置 p 加入莫队窗口，并在该值首次出现时增加 distinct。
// 参数：要加入窗口的数组位置 p。
// 变量：add=把数组位置加入当前莫队窗口的局部 lambda add。
auto add = [&](int p) { if (++cnt[a[p]] == 1) ++distinct; };
// 接口：remove(p)：把位置 p 移出莫队窗口，并在该值不再出现时减少 distinct。
// 参数：要移出窗口的数组位置 p。
// 变量：remove=把位置移出当前窗口的局部 lambda remove。
auto remove = [&](int p) { if (--cnt[a[p]] == 0) --distinct; };
// 变量：qu=当前离线查询 qu。
for (auto qu : qs) {
    while (L > qu.l) add(--L); while (R < qu.r) add(++R);
    while (L < qu.l) remove(L++); while (R > qu.r) remove(R--);
    answer[qu.id] = distinct;
}
\end{lstlisting}

\section{平衡树思想（了解 Treap）}\label{ux5e73ux8861ux6811ux601dux60f3ux4e86ux89e3-treap}

随机优先级的 BST，支持按键插入、删除、排名、第 k 小、分裂/合并，期望 \passthrough{\lstinline!O(log n)!}。无序序列维护时使用 FHQ Treap，节点存子树大小、区间反转标记、区间和/最大值。需要严格稳定最坏复杂度时优先线段树或 \passthrough{\lstinline!std::set!}。

\begin{lstlisting}[language={C++}]
struct Node { int l, r, key, pri, sz; } tr[MAXN]; // 儿子、键、随机优先级、子树大小。
// 变量：root=当前树结构的根节点编号 root；tot=当前已经创建的数据结构节点数量 tot。
int root, tot;
// 接口：size(u)：返回节点 u 所代表子树的节点数，空节点 0 的大小为 0；返回类型 int。
// 参数：待统计大小的 Treap 子树根 u。
int size(int u) { return u ? tr[u].sz : 0; }
// 接口：pull(u)：由左右孩子重算 Treap 节点 u 的子树大小；会修改当前结构；多测时重新构造或显式清空成员。
// 参数：待重算大小的 Treap 节点 u。
void pull(int u) { tr[u].sz = size(tr[u].l) + size(tr[u].r) + 1; }
// 接口：split(u, key, x, y)：按键值 key 把 Treap u 拆成键值不大于 key 的左树 x 与其余右树 y；不返回值。
// 参数：待拆分的 Treap 根 u；拆分键 key；接收拆分后左 Treap 根的引用 x；接收拆分后右 Treap 根的引用 y。
void split(int u, int key, int &x, int &y) {
    if (!u) return x = y = 0, void();
    if (tr[u].key <= key) split(tr[u].r, key, tr[u].r, y), x = u;
    else split(tr[u].l, key, x, tr[u].l), y = u;
    pull(u);
}
// 接口：merge(x, y)：按优先级合并键值有序的左右 Treap，并返回新根；返回类型 int。
// 参数：待合并的左 Treap 根 x；待合并的右 Treap 根 y。
int merge(int x, int y) {
    if (!x || !y) return x ? x : y;
    if (tr[x].pri > tr[y].pri) return tr[x].r = merge(tr[x].r, y), pull(x), x;
    return tr[y].l = merge(x, tr[y].l), pull(y), y;
}
\end{lstlisting}

\section{撤销与可回滚结构}\label{ux64a4ux9500ux4e0eux53efux56deux6edaux7ed3ux6784}

不做路径压缩的 DSU 可用栈记录每次修改，支持回滚到快照 \passthrough{\lstinline!O(log n)!} 合并、\passthrough{\lstinline!O(1)!} 回滚；配合线段树分治时间解决``边在一段时间内存在''的动态连通性。

\begin{lstlisting}[language={C++}]
struct RollbackDSU {
    vector<int> p, sz; // p[x]=父节点；sz[root]=集合大小；不做路径压缩。
    vector<pair<int, int>> hist; // 每次成功合并的撤销记录；长度就是快照。
  // 接口：RollbackDSU(n)：构造支持快照和回滚、且不做路径压缩的并查集；构造并初始化对象；每组测试建议重新构造。
  // 参数：可回滚并查集管理的元素编号上界 n。
    RollbackDSU(int n): p(n + 1), sz(n + 1, 1) { iota(p.begin(), p.end(), 0); }
    // 接口：find(x)：返回 x 当前所在并查集的代表元；返回类型 int。
    // 参数：待查询代表元的元素编号 x。
    int find(int x) { while (x != p[x]) x = p[x]; return x; }
    // 接口：snapshot()：返回当前撤销栈长度，供 rollback 恢复；返回类型 int。
    int snapshot() { return hist.size(); }
    // 接口：unite(a, b)：合并两个元素所在集合；原本同集合时返回 false；实际合并两个集合时返回 true。
    // 参数：第一个待合并元素编号 a；第二个待合并元素编号 b。
    bool unite(int a, int b) {
        a = find(a); b = find(b); if (a == b) return false;
        if (sz[a] < sz[b]) swap(a, b);
        hist.push_back({b, sz[a]}); p[b] = a; sz[a] += sz[b]; return true;
    }
    // 接口：rollback(snap)：把可撤销数据结构恢复到指定快照；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：此前 snapshot() 返回的撤销栈长度 snap。
    void rollback(int snap) {
        while ((int)hist.size() > snap) {
            auto [b, old] = hist.back(); hist.pop_back();
            sz[p[b]] = old; p[b] = b;
        }
    }
};
\end{lstlisting}

\section{李超线段树（Li Chao Segment Tree）}\label{ux674eux8d85ux7ebfux6bb5ux6811li-chao-segment-tree}

维护直线集合在离散/整数横坐标上的最值。每个节点保存当前区间中在中点最优的直线，递归把另一条直线送入可能胜出的子区间；插入和查询均为 \passthrough{\lstinline!O(log V)!}。斜率、截距和查询坐标使用 \passthrough{\lstinline!long long!}，求值与查询结果统一使用 \passthrough{\lstinline!\_\_int128!}，因此不会在乘加处发生窄整数溢出。

\begin{lstlisting}[language={C++}]
struct LiChao {
    using ll = long long;
    struct Line {
        // 这份代码维护最小值；显式 empty 标记不会与任何合法直线值冲突。
        // 变量：k=空李超树节点使用的直线斜率 k；b=空李超树节点使用的直线截距 b。
        ll k = 0, b = 0;
        bool empty = true; // 当前节点是否尚未保存真实直线。
        // 接口：Line()：构造一条空直线；构造并初始化对象。
        Line() = default;
        // 接口：Line(k, b)：构造斜率 k、截距 b 的真实直线；构造并初始化对象。
        // 参数：直线斜率 k；直线截距 b。
        Line(ll k, ll b): k(k), b(b), empty(false) {}
        // 接口：get(x)：返回直线在横坐标 x 处的函数值；返回类型 __int128。
        // 参数：计算直线函数值的横坐标 x。
        __int128 get(ll x) const {
            return empty ? ((__int128)1 << 126) : (__int128)k * x + b;
        }
    };
    int L, R;             // 允许查询的整数横坐标闭区间 [L,R]。
    vector<Line> tr;      // tr[p]：节点区间内当前保留下来的候选直线。
    // 接口：storage_size(L, R)：返回整数闭区间 [L,R] 的静态李超树数组长度。
    // 参数：闭区间左端点 L；闭区间右端点 R，要求 L<=R。
    static size_t storage_size(int L, int R) {
        if (L > R) throw std::invalid_argument("empty Li Chao domain");
        return 4ULL * ((long long)R - L + 1);
    }
  // 接口：LiChao(L, R)：构造整数横坐标闭区间 [L,R] 上的李超树；构造并初始化对象；每组测试建议重新构造。
  // 参数：闭区间左端点 L；闭区间右端点 R。
    LiChao(int L, int R): L(L), R(R), tr(storage_size(L, R)) {}

    // 接口：add_line(nw, p, l, r)：把候选直线 nw 插入整数横坐标闭区间 [l,r] 的李超树节点 p；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：待插入李超树的直线 nw；当前节点 u 的父节点 p；闭区间左端点 l；闭区间右端点 r。
    void add_line(Line nw, int p, int l, int r) {
        // 先比较新线和当前线在端点/中点的优劣。
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        int m = l + (int)(((long long)r - l) / 2);
        // 变量：lef=新直线在区间左端是否更优 lef。
        bool lef = nw.get(l) < tr[p].get(l);
        // 变量：mid=当前区间中点 mid。
        bool mid = nw.get(m) < tr[p].get(m);

        // 中点更优的线留在当前节点，旧线递归到可能赢的半边。
        if (mid) swap(nw, tr[p]);
        if (l == r) return;

        // 两线在左端和中点的相对优劣发生变化，交点在左半。
        if (lef != mid) add_line(nw, p * 2, l, m);
        else add_line(nw, p * 2 + 1, m + 1, r);
    }
    // 对外接口：只需要传入斜率 k 和截距 b，不要手动传根节点参数。
    // 接口：add_line(k, b)：把斜率 k、截距 b 组成的直线插入李超树；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：待插入直线的斜率 k；待插入直线的截距 b。
    void add_line(ll k, ll b) { add_line({k, b}, 1, L, R); }

    // 接口：query(x, p, l, r)：返回横坐标 x 处沿李超树查询路径得到的最优直线函数值；返回类型 __int128。
    // 参数：计算候选直线最优值的横坐标 x；当前线段树节点编号 p（根通常为 1）；闭区间左端点 l；闭区间右端点 r。
    __int128 query(ll x, int p, int l, int r) const {
        // x 所在路径上的每个节点都可能保存一条更优直线，必须全部比较。
        // 变量：ans=李超树当前节点及递归路径在横坐标 x 处的最优函数值 ans。
        __int128 ans = tr[p].get(x);
        if (l == r) return ans;
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        int m = l + (int)(((long long)r - l) / 2);
        if (x <= m) return min(ans, query(x, p * 2, l, m));
        return min(ans, query(x, p * 2 + 1, m + 1, r));
    }
    // x 必须落在构造时给出的 [L,R] 内；大坐标范围应改用动态开点版本。
    // 接口：query(x)：返回李超树中全部候选直线在横坐标 x 处的最优函数值；返回类型 __int128。
    // 参数：计算候选直线最优值的横坐标 x。
    __int128 query(ll x) const { return query(x, 1, L, R); }
};
\end{lstlisting}

求最大值时把比较符号整体反向。若只会在若干离散横坐标上查询，可以另存有序真实坐标 \passthrough{\lstinline!xs!}，递归用下标分区，但直线求值必须代入 \passthrough{\lstinline!xs[mid]!} 与 \passthrough{\lstinline!xs[index]!}，不能把压缩编号当成真实横坐标。

\section{区间最值势能线段树（Segment Tree Beats）}\label{ux533aux95f4ux6700ux503cux52bfux80fdux7ebfux6bb5ux6811segment-tree-beats}

``区间取 \passthrough{\lstinline!min!} + 区间和/最大值''不能用普通懒标记，因为只有当前最大值会被压低。维护最大值、次大值、最大值出现次数和区间和；当 \passthrough{\lstinline!x!} 严格大于次大值时可以整段打标记。

\begin{lstlisting}[language={C++}]
struct SegBeats {
    using ll = long long;
    // 变量：NEG=不存在次大值时使用的 __int128 负无穷哨兵 NEG。
    static constexpr __int128 NEG = -((__int128)1 << 126);
    int n; // 原数组及所有公开区间接口均使用 1-based 闭区间 [1,n]。
    vector<__int128> sum; // 精确区间和，允许超过 long long 范围。
    vector<ll> mx;        // 区间最大值。
    vector<__int128> second_mx; // 严格小于最大值的次大值；宽类型使哨兵不与 ll 合法值冲突。
    vector<int> cnt_mx;   // 最大值出现次数。
  // 接口：SegBeats(a)：由 1-based 数组 a 构造区间 chmin + 区间和线段树；构造并初始化对象；每组测试建议重新构造。
  // 参数：用于构造线段树的 1-based 初始数组 a。
    SegBeats(const vector<ll>& a): n((int)a.size() - 1),
        sum(4 * n + 4), mx(4 * n + 4), second_mx(4 * n + 4),
        cnt_mx(4 * n + 4) { build(1, 1, n, a); }

    // 接口：pull(p)：由两个孩子重算线段树节点 p 的聚合信息；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：当前线段树节点编号 p（根通常为 1）。
    void pull(int p) {
        // 父节点只保存“最大值这一类”和“次大值这一类”，
        // 不需要保存整个儿子数组，因此 chmin 才能整段快速处理。
        sum[p] = sum[p * 2] + sum[p * 2 + 1];
        if (mx[p * 2] == mx[p * 2 + 1]) {
            mx[p] = mx[p * 2]; cnt_mx[p] = cnt_mx[p * 2] + cnt_mx[p * 2 + 1];
            second_mx[p] = max(second_mx[p * 2], second_mx[p * 2 + 1]);
        } else {
            // 变量：x=线段树节点 p 的左儿子编号 x；y=线段树节点 p 的右儿子编号 y。
            int x = p * 2, y = p * 2 + 1;
            if (mx[x] < mx[y]) swap(x, y);
            mx[p] = mx[x]; cnt_mx[p] = cnt_mx[x];
            second_mx[p] = max(second_mx[x], (__int128)mx[y]);
        }
    }
    // 接口：build(p, l, r, a)：由 1-based 数组 a 建立区间 chmin 与区间和线段树；完成初始化；多测时每组重新调用。
    // 参数：当前线段树节点编号 p（根通常为 1）；闭区间左端点 l；闭区间右端点 r；用于构造当前数据结构的初始数组 a。
    void build(int p, int l, int r, const vector<ll>& a) {
        if (l == r) {
            // 叶子只有一个最大值，次大值设为负无穷。
            sum[p] = mx[p] = a[l]; second_mx[p] = NEG; cnt_mx[p] = 1;
            return;
        }
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        int m = (l + r) / 2;
        build(p * 2, l, m, a); build(p * 2 + 1, m + 1, r, a); pull(p);
    }
    // 接口：apply_chmin(p, x)：给当前线段树节点应用 chmin 标记；不返回值。
    // 参数：当前线段树节点编号 p（根通常为 1）；节点最大值要压低到的上界 x。
    void apply_chmin(int p, ll x) {
        // 调用者保证 second_mx[p] < x < mx[p]，
        // 因而只有 cnt_mx 个最大值会下降，sum 可以 O(1) 更新。
        if (x >= mx[p]) return;
        sum[p] -= ((__int128)mx[p] - x) * cnt_mx[p];
        mx[p] = x;
    }
    // 接口：push(p)：把 Segment Tree Beats 节点 p 的最大值上界约束下传给两个孩子；会修改当前结构；多测时重新构造或显式清空成员。
    // 参数：当前线段树节点编号 p（根通常为 1）。
    void push(int p) {
        // 父节点的 mx 变小后，儿子的最大值可能仍比父亲大，
        // 把父亲的上界传给儿子即可，不需要递归到底。
        if (mx[p * 2] > mx[p]) apply_chmin(p * 2, mx[p]);
        if (mx[p * 2 + 1] > mx[p]) apply_chmin(p * 2 + 1, mx[p]);
    }
    // 接口：chmin(p, l, r, ql, qr, x)：把目标闭区间内每个值更新为 min(原值,x)；不返回值。
    // 参数：当前节点 u 的父节点 p；闭区间左端点 l；闭区间右端点 r；目标闭区间左端 ql；目标闭区间右端 qr；目标区间元素要压低到的上界 x。
    void chmin(int p, int l, int r, int ql, int qr, ll x) {
        // 无交集或当前最大值已经不超过 x 时，操作没有影响。
        if (qr < l || r < ql || x >= mx[p]) return;
        // x 大于次大值时只有最大值受影响，触发势能线段树的核心优化。
        if (ql <= l && r <= qr && x > second_mx[p]) return apply_chmin(p, x);
        // 否则必须下传并继续递归；删掉这个条件会退化甚至超时。
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        push(p); int m = (l + r) / 2;
        chmin(p * 2, l, m, ql, qr, x);
        chmin(p * 2 + 1, m + 1, r, ql, qr, x); pull(p);
    }
    // 接口：query_sum(p, l, r, ql, qr)：返回闭区间查询的元素和；返回类型 __int128。
    // 参数：当前节点 u 的父节点 p；闭区间左端点 l；闭区间右端点 r；目标闭区间左端 ql；目标闭区间右端 qr。
    __int128 query_sum(int p, int l, int r, int ql, int qr) {
        if (qr < l || r < ql) return 0;
        if (ql <= l && r <= qr) return sum[p];
        // 变量：m=当前递归区间 [l,r] 的中点 m。
        push(p); int m = (l + r) / 2;
        return query_sum(p * 2, l, m, ql, qr)
             + query_sum(p * 2 + 1, m + 1, r, ql, qr);
    }
    // 接口：chmin(l, r, x)：把目标闭区间内每个值更新为 min(原值,x)；不返回值。
    // 参数：闭区间左端点 l；闭区间右端点 r；目标区间元素要压低到的上界 x。
    void chmin(int l, int r, ll x) { chmin(1, 1, n, l, r, x); }
    // 接口：query_sum(l, r)：返回闭区间查询的精确元素和；返回类型 __int128。
    // 参数：闭区间左端点 l；闭区间右端点 r。
    __int128 query_sum(int l, int r) { return query_sum(1, 1, n, l, r); }
};
\end{lstlisting}
