...
// 接口：sweep_area(events, covered_length, update, y_id)：按 x 扫描事件并返回覆盖面积；空事件返回 0。
// 参数：covered_length 返回当前 y 覆盖长度；update 接收离散 y 区间与增量；y_id 返回 y 的离散下标。
template<class CoveredLength, class Update, class YId>
long long sweep_area(vector<Event> events, CoveredLength covered_length,
    ...
    // 变量：area=已经扫过的覆盖面积；last_x=上一批事件的横坐标。
...
\begin{lstlisting}[language={C++}]
// 接口：normalize_direction(dx, dy)：把非零整数方向约为互质且符号唯一的 (dx,dy)。
pair<long long, long long> normalize_direction(long long dx, long long dy) {
    if (dx == 0 && dy == 0) throw invalid_argument("zero direction");
    // 变量：g=两个方向分量绝对值的最大公约数。
...
// 接口：safe_lcm(a, b)：返回非负最小公倍数；0 与任意数的最小公倍数为 0。
long long safe_lcm(long long a, long long b) {
    if (a == 0 || b == 0) return 0;
    return llabs(a / std::gcd(a, b) * b);
}
\end{lstlisting}
