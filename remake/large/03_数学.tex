...
// 参数：扩展欧几里得的第一个整数输入 a；扩展欧几里得的第二个整数输入 b；输出引用 x，满足 a*x+b*y=gcd(a,b)；输出引用 y，满足 a*x+b*y=gcd(a,b)。
...
fac[0] = 1;
// 变量：i=阶乘表正在预处理的整数下标 i。
for (int i = 1; i < N; ++i) fac[i] = fac[i - 1] * i % MOD;
ifac[N - 1] = qpow(fac[N - 1], MOD - 2, MOD);
// 变量：i=逆阶乘表自后向前递推的整数下标 i。
for (int i = N - 1; i; --i) ifac[i - 1] = ifac[i] * i % MOD;
// 接口：C(n, k)：返回预处理阶乘下的组合数 C(n,k) mod MOD，越界时返回 0。
// 参数：组合数上标 n；组合数下标 k。
...
// 参数：组合数 C(n,k) 的上标 n；组合数 C(n,k) 的下标 k；组合数运算使用的质数模数 p。
long long Csmall(long long n, long long k, long long p) {
    if (k < 0 || k > n) return 0;
    // 变量：ans=逐项乘除计算组合数 C(n,k) 的累积结果 ans。
    long long ans = 1;
    // 变量：i=小组合数 C(n,k) 逐项乘除时的因子下标 i。
...
// 参数：组合数 C(n,k) 的上标 n；组合数 C(n,k) 的下标 k；Lucas 定理使用的质数模数 p。
...
// 接口：merge_congruence(a1, m1, a2, m2, a, m)：合并两个同余方程，结果写入 a,m；方程相容并完成合并时返回 true。
// 参数：第一个同余式的余数 a1；第一个同余式的正模数 m1；第二个同余式的余数 a2；第二个同余式的正模数 m2；合并后余数的输入输出引用 a；合并后正模数的输入输出引用 m。
bool merge_congruence(long long a1, long long m1,
                      long long a2, long long m2,
                      long long &a, long long &m) {
    // 变量：x=exgcd 返回、满足 m1*x+m2*y=gcd(m1,m2) 的 Bézout 系数 x；y=exgcd 返回、满足 m1*x+m2*y=gcd(m1,m2) 的 Bézout 系数 y；g=递归返回的 gcd(a,b) 值 g。
    long long x, y, g = exgcd(m1, m2, x, y);
    // 变量：d=两个同余方程余数之差 d=a2-a1。
    long long d = a2 - a1;
    if (d % g) return false;
    // 变量：mod=当前正模数 mod。
    long long mod = m2 / g;
    // 变量：t=CRT 合并时由 Bézout 系数求出的第二个模数倍数 t。
...
struct Mat {
  ...
  // 参数：矩阵阶数 n；是否构造单位矩阵 ident。
    Mat(int n, bool ident = false): n(n), a(n, vector<long long>(n)) {
        // 变量：i=单位矩阵当前写入对角元的行列下标 i。
        if (ident) for (int i = 0; i < n; ++i) a[i][i] = 1;
    }
};
// 接口：operator*(A, B)：返回输入矩阵 A 与 B 的矩阵乘积；返回类型 Mat。
// 参数：矩阵乘法的第一个输入矩阵 A；矩阵乘法的第二个输入矩阵 B。
Mat operator*(const Mat& A, const Mat& B) {
    // 变量：C=矩阵乘法正在累加构造的结果矩阵 C。
    Mat C(A.n);
    // 变量：i=矩阵乘法结果矩阵的行下标 i。
    for (int i = 0; i < A.n; ++i)
        // 变量：k=矩阵乘法中连接行 i 与列 j 的公共维下标 k。
...
// 变量：ans=容斥枚举所有非空集合后累计的带符号计数 ans。
__int128 ans = 0;
...
pair<long long, long long> fib(long long n) { // (F_n, F_{n+1})
    if (!n) return {0, 1};
    auto [a, b] = fib(n >> 1);
    // 变量：c=快速倍增公式得到的 F(2k) 值 c。
    long long c = a * (2 * b - a);
    // 变量：d=快速倍增公式得到的 F(2k+1) 值 d。
    long long d = a * a + b * b;
...
// 参数：卷积的左系数数组 a；卷积的右系数数组 b；运算使用的正模数 MOD。
Poly multiply(const Poly& a, const Poly& b, long long MOD) {
    // 变量：c=存放卷积系数的结果多项式 c。
    Poly c(a.size() + b.size() - 1);
    // 变量：i=多项式 a 当前参与卷积的系数下标 i。
    for (int i = 0; i < (int)a.size(); ++i)
        // 变量：j=多项式 b 当前参与卷积的系数下标 j。
...
// 参数：幂运算的底多项式 base；计数模型中需要选择的元素数 k；运算使用的正模数 MOD。
Poly ways(Poly base, int k, long long MOD) {
    // 变量：ans=多项式快速幂中当前累计乘积 ans。
    Poly ans{1};
    while (k) {
...
long long burnside(const vector<vector<int>>& group,
    ...
    // 变量：sum=Burnside 引理对每个群元素固定染色数的累加和 sum。
    long long sum = 0;
    for (const auto& perm : group) {
        // 变量：n=容器 perm 的元素数量 n；cycles=当前置换的环数量 cycles。
        int n = perm.size(), cycles = 0;
        // 变量：vis=分解当前群作用置换时记录位置是否已进入某个环的标记 vis。
        vector<char> vis(n);
        // 变量：i=当前群作用置换中尚未归入置换环的位置 i。
        for (int i = 0; i < n; ++i) if (!vis[i]) {
            ++cycles;
            // 变量：u=沿当前置换环移动的元素编号 u。
...
vector<int> prufer_encode(const vector<vector<int>>& g) {
    ...
    // 变量：step=Prüfer 编码当前删除第几个叶子的步骤下标 step。
    for (int step = 0; step < n - 2; ++step) {
        // 变量：leaf=当前取出的最小叶子 leaf。
        int leaf = leaves.top(); leaves.pop();
        // 变量：v=Prüfer 编码中与当前叶子相邻的未删除顶点 v。
        int v = 0;
        // 变量：x=从 g[leaf] 枚举的相邻顶点 x。
...
long long tree_count(vector<vector<long long>> lap, long long MOD) {
    // 变量：n=容器 lap 的元素数量 n；det=高斯消元过程中累计的行列式 det。
    int n = lap.size(); long long det = 1;
    // 变量：col=当前高斯消元处理的列 col。
    for (int col = 0; col < n; ++col) {
        ...
        // 变量：inv=当前运算使用的逆元 inv。
        long long inv = mod_pow((lap[col][col] % MOD + MOD) % MOD, MOD - 2, MOD);
        // 变量：i=模高斯消元中在主元行下方寻找非零元的候选行 i。
        for (int i = col + 1; i < n; ++i) {
            // 变量：factor=当前步骤使用的乘数 factor。
            long long factor = lap[i][col] % MOD * inv % MOD;
            // 变量：j=模高斯消元中从主元列 col 开始更新的列下标 j。
            for (int j = col; j < n; ++j)
...
// 参数：待判断二次剩余性的整数 a；Legendre 符号使用的奇质数模数 p。
long long legendre(long long a, long long p) {
    a %= p; if (a < 0) a += p;
    if (!a) return 0;
    // 变量：x=Euler 判别得到的 a^((p-1)/2) mod p 值 x。
    long long x = mod_pow(a, (p - 1) / 2, p);
    return x == 1 ? 1 : -1;
}

// 接口：tonelli_shanks(n, p)：求 x^2=n (mod p) 的一个根，无解返回 -1；返回类型 long long。
// 参数：需要求模平方根的剩余 n；需要求平方根的奇质数模数 p。
long long tonelli_shanks(long long n, long long p) {
    n %= p; if (!n) return 0;
    if (p == 2) return n;
    if (legendre(n, p) != 1) return -1;
    if (p % 4 == 3) return mod_pow(n, (p + 1) / 4, p);
    // 变量：q=分解 p-1=q*2^s 后得到的奇数因子 q；s=分解 p-1=q*2^s 时因子 2 的指数 s。
    long long q = p - 1, s = 0;
    while (!(q & 1)) q >>= 1, ++s;
    // 变量：z=Tonelli-Shanks 正在试探的最小二次非剩余候选 z。
    long long z = 2;
    while (legendre(z, p) != -1) ++z;
    // 变量：c=Tonelli-Shanks 当前 2 幂阶生成元 c；x=Tonelli-Shanks 当前平方根候选 x。
    long long c = mod_pow(z, q, p), x = mod_pow(n, (q + 1) / 2, p);
    // 变量：t=Tonelli-Shanks 当前误差项 t；m=Tonelli-Shanks 当前误差项的 2 幂阶上界 m。
    long long t = mod_pow(n, q, p), m = s;
    while (t != 1) {
        // 变量：i=Tonelli-Shanks 中寻找 t^(2^i)=1 的最小指数 i；tt=当前平方根修正量 t 的平方 tt。
...
\begin{lstlisting}[language={C++}]
// 变量：vis=置换环分解中记录每个位置是否已经处理的标记 vis；result=原置换重复应用 k 次后每个位置的映射结果 result。
vector<int> vis(n), result(n);
// 变量：i=置换环分解中当前尚未访问的位置 i。
for (int i = 0; i < n; ++i) if (!vis[i]) {
    // 变量：cyc=当前置换环按遍历顺序保存的全部元素编号 cyc。
    vector<int> cyc;
    // 变量：u=沿置换 p 当前环移动的元素编号 u。
...
// 接口：ntt(a, inv)：对数组 a 做模 998244353 的 NTT；invert=true 时做逆变换；不返回值。
// 参数：原地执行正变换或逆变换的系数数组 a；是否执行逆变换 inv。
void ntt(vector<int>& a, bool inv) {
    // 变量：n=容器 a 的元素数量 n。
    int n = a.size();
    // 变量：i=NTT 位逆序置换中当前待重排的系数下标 i。
    for (int i = 1, j = 0; i < n; ++i) {
        // 变量：bit=NTT 位逆序置换中从最高有效位开始移动的掩码 bit。
        int bit = n >> 1; for (; j & bit; bit >>= 1) j ^= bit; j ^= bit;
        if (i < j) swap(a[i], a[j]);
    }
    // 变量：len=NTT 当前蝶形合并块的长度 len。
    for (int len = 2; len <= n; len <<= 1) {
        // 变量：wlen=当前 NTT 蝶形层使用的单位根 wlen。
        int wlen = qpow(NTT_G, (NTT_MOD - 1) / len, NTT_MOD);
        if (inv) wlen = qpow(wlen, NTT_MOD - 2, NTT_MOD);
        // 变量：i=NTT 长度为 len 的蝶形块起始下标 i。
        for (int i = 0; i < n; i += len) {
            // 变量：w=NTT 当前蝶形位置使用的单位根幂 w。
            long long w = 1;
            // 变量：j=NTT 长度为 len 的蝶形块内偏移下标 j。
            for (int j = 0; j < len / 2; ++j) {
                // 变量：u=NTT 蝶形运算左半位置的旧系数 u；v=NTT 蝶形运算右半位置乘单位根后的系数 v。
...
int gauss(vector<vector<long long>> a) {
    // 变量：n=增广矩阵的行数 n；m=未知量列数 m；rank=高斯消元已经确定主元的行数，即当前矩阵秩 rank。
    ...
    for (int col = 0; col < m && rank < n; ++col) {
        ...
        // 变量：j=高斯消元中从主元列 col 开始更新的增广矩阵列下标 j。
        for (int j = col; j < m; ++j)
            a[rank][j] = (__int128)a[rank][j] * inv % MOD;
        // 变量：i=模高斯消元中当前用主元行消去的其它行下标 i。
        for (int i = 0; i < n; ++i) if (i != rank && a[i][col]) {
            // 变量：t=高斯消元当前行在主元列上的消元系数 t。
            long long t = a[i][col];
            // 变量：j=高斯消元中从主元列 col 开始更新的增广矩阵列下标 j。
            for (int j = col; j < m; ++j)
