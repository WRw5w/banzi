...
`framework` 还必须填写 `structural_contract`、逐块 `required_patterns`、至少两项
`missing_dependencies` 以及实际检查面的 `validated_surface`。`note` 必须逐项点名这些
缺失依赖；门禁会剥除块 ID、标题和通用报告尾句后比较理由，拒绝只换 ID/标题的近似
套话。门禁同时拒绝占位符、语法破损、未声明的空钩子、恒定返回算法空壳和结构漂移，
并真实调用编译器验证无效反例会失败。它只确认已登记框架仍保持声明的结构，不冒充
编译或算法语义测试。已经有关联差分/契约证据的代码块不能再重复分类。
