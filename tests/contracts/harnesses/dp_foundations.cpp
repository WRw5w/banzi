...
pair<int,long long> run(const vector<int>& input){const auto&a=input;int n=a.size();
// @@@LIS_MAX_SUBARRAY@@@
...
int main(){mt19937 rng(20260829);
    for(int round=0;round<2000;++round){int n=1+rng()%40;vector<int>a(n);for(int&x:a)x=(int)(rng()%51)-25;auto got=linear_dp_case::run(a);int lis=0;vector<int>d(n,1);long long sub=LLONG_MIN;for(int i=0;i<n;++i){for(int j=0;j<i;++j)if(a[j]<a[i])d[i]=max(d[i],d[j]+1);lis=max(lis,d[i]);long long s=0;for(int j=i;j<n;++j)s+=a[j],sub=max(sub,s);}if(got!=make_pair(lis,sub))return 1;}
    ...
    for(int round=0;round<1000;++round){int n=1+rng()%100,k=1+rng()%20;vector<long long>base(n);for(auto&x:base)x=(int)(rng()%31)-15;auto got=window_dp_case::window_min_dp(base,k),want=base;for(int i=1;i<n;++i){long long best=1LL<<60;for(int j=max(0,i-k);j<i;++j)best=min(best,want[j]);want[i]+=best;}if(got!=want)return 6;}
